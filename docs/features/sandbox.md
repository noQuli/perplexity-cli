# Sandboxing in Perplexity CLI

This document provides a guide to sandboxing in Perplexity CLI, including prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up Perplexity CLI:
...
perplexity -s -p "analyze the code structure"

# Use environment variable
export PERPLEXITY_SANDBOX=true
perplexity -p "run the test suite"
...
2. **Environment variable**: `PERPLEXITY_SANDBOX=true|docker|podman|sandbox-exec`
...
```bash
DEBUG=1 perplexity -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.perplexity/.env` files for Perplexity CLI-specific debug settings.

### Inspect sandbox

```bash
# Check environment
perplexity -s -p "run shell command: env | grep SANDBOX"

# List mounts
perplexity -s -p "run shell command: mount | grep workspace"
```
