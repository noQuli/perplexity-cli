...
Subagents are specialized AI assistants that handle specific types of tasks within Perplexity CLI. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
- **Project-level**: `.perplexity/agents/` (takes precedence)
- **User-level**: `~/.perplexity/agents/` (fallback)
...
Perplexity CLI proactively delegates tasks based on:
