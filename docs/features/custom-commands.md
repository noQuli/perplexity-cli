...
prompts as personal shortcuts within Perplexity CLI. You can create commands that
...
1.  **User commands (global):** Located in `~/.perplexity/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.perplexity/commands/`. These commands are specific to the
...
- A file at `<project>/.perplexity/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the Perplexity model when the
...
When a custom command attempts to execute a shell command, Perplexity CLI will now
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.perplexityignore` if enabled.
...
Open `~/.perplexity/commands/refactor/pure.toml` in your editor and add the
