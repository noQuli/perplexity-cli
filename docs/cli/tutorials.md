...
This page contains tutorials for interacting with Perplexity CLI.
...
In your project's root directory, create or open the [`.perplexity/settings.json` file](./configuration.md). Within the file, add the `mcpServers` configuration block, which provides instructions for how to launch the GitHub MCP server.
...
Perplexity CLI uses this value in the `mcpServers` configuration that you defined in the `settings.json` file.

#### Launch Perplexity CLI and verify the connection

When you launch Perplexity CLI, it automatically reads your configuration and launches the GitHub MCP server in the background. You can then use natural language prompts to ask Perplexity CLI to perform GitHub actions. For example:
