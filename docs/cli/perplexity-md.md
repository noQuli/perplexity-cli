...
Context files, which use the default name `PERPLEXITY.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.perplexity/PERPLEXITY.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `PERPLEXITY.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `PERPLEXITY.md` files in subdirectories
      below your current working directory. It respects rules in `.gitignore`
      and `.perplexityignore`.
...
### Example `PERPLEXITY.md` file

Here is an example of what you can include in a `PERLEXITY.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `PERPLEXITY.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.perplexity/PERPLEXITY.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `PERPLEXITY.md` files into smaller, more manageable
...
**Example `PERPLEXITY.md` with imports:**
...
While `PERPLEXITY.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "PERPLEXITY.md"]
  }
}
```
