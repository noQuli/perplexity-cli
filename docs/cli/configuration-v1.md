...
Perplexity CLI offers several ways to configure its behavior, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings.
...
Perplexity CLI uses JSON settings files for persistent configuration. There are four locations for these files:

- **System defaults file:**
  - **Location:** `/etc/perplexity-cli/system-defaults.json` (Linux), `C:\ProgramData\perplexity-cli\system-defaults.json` (Windows) or `/Library/Application Support/PerplexityCode/system-defaults.json` (macOS). The path can be overridden using the `PERPLEXITY_CODE_SYSTEM_DEFAULTS_PATH` environment variable.
...
- **User settings file:**
  - **Location:** `~/.perplexity/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Perplexity CLI sessions for the current user.
- **Project settings file:**
  - **Location:** `.perplexity/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Perplexity CLI from that specific project. Project settings override user settings.

- **System settings file:**
  - **Location:** `/etc/perplexity-cli/settings.json` (Linux), `C:\ProgramData\perplexity-cli\settings.json` (Windows) or `/Library/Application Support/PerplexityCode/settings.json` (macOS). The path can be overridden using the `PERPLEXITY_CODE_SYSTEM_SETTINGS_PATH` environment variable.
  - **Scope:** Applies to all Perplexity CLI sessions on the system, for all users. System settings override user and project settings. May be useful for system administrators at enterprises to have controls over users' Perplexity CLI setups.

**Note on environment variables in settings:** String values within your `settings.json` files can reference environment variables using either `$VAR_NAME` or `${VAR_NAME}` syntax. These variables will be automatically resolved when the settings are loaded. For example, if you have an environment variable `MY_API_TOKEN`, you could use it in `settings.json` like this: `"apiKey": "$MY_API_TOKEN"`.

### The `.perplexity` directory in your project

In addition to a project settings file, a project's `.perplexity` directory can contain other project-specific files related to Perplexity CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g., `.perplexity/sandbox-macos-custom.sb`, `.perplexity/sandbox.Dockerfile`).

### Available settings in `settings.json`:

- **`contextFileName`** (string or array of strings):
  - **Description:** Specifies the filename for context files (e.g., `PERPLEXITY.md`, `AGENTS.md`). Can be a single filename or a list of accepted filenames.
  - **Default:** `PERPLEXITY.md`
  - **Example:** `"contextFileName": "AGENTS.md"`

- **`bugCommand`** (object):
  - **Description:** Overrides the default URL for the `/bug` command.
  - **Default:** `"urlTemplate": "https://github.com/NoQuli/perplexity-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
1.  **Use `.perplexityignore`:** Create a `.perplexityignore` file in your project root to exclude directories that contain a large number of files that you don't need to reference (e.g., build artifacts, logs, `node_modules`). Reducing the total number of files crawled is the most effective way to improve performance.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for Perplexity CLI.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool execution. If set to `true`, Perplexity CLI uses a pre-built `perplexity-cli-sandbox` Docker image. For more information, see [Sandboxing](#sandboxing).
  - **Default:** `false`
  - **Example:** `"sandbox": "docker"`

- **`toolDiscoveryCommand`** (string):
  - **Description:** **Align with Perplexity CLI.** Defines a custom shell command for discovering tools from your project. The shell command must return on `stdout` a JSON array of [function declarations](https://ai.google.dev/perplexity-api/docs/function-calling#function-declarations). Tool wrappers are optional.
  - **Default:** Empty
  - **Example:** `"toolDiscoveryCommand": "bin/get_tools"`

- **`toolCallCommand`** (string):
  - **Description:** **Align with Perplexity CLI.** Defines a custom shell command for calling a specific tool that was discovered using `toolDiscoveryCommand`. The shell command must meet the following criteria:
    - It must take function `name` (exactly as in [function declaration](https://ai.google.dev/perplexity-api/docs/function-calling#function-declarations)) as first command line argument.
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context Protocol (MCP) servers for discovering and using custom tools. Perplexity CLI attempts to connect to each configured MCP server to discover available tools. If multiple MCP servers expose a tool with the same name, the tool names will be prefixed with the server alias you defined in the configuration (e.g., `serverAlias__actualToolName`) to avoid conflicts. Note that the system might strip certain schema properties from MCP tool definitions for compatibility. At least one of `command`, `url`, or `httpUrl` must be provided. If multiple are specified, the order of precedence is `httpUrl`, then `url`, then `command`.
  - **Default:** Empty
  - **Properties:**
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Perplexity CLI. For more information, see [Telemetry](../telemetry.md).
...
- **`excludedProjectEnvVars`** (array of strings):
  - **Description:** Specifies environment variables that should be excluded from being loaded from project `.env` files. This prevents project-specific environment variables (like `DEBUG=true`) from interfering with the CLI behavior. Variables from `.perplexity/.env` files are never excluded.
...
- **`loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls the behavior of the `/memory refresh` command. If set to `true`, `PERPLEXITY.md` files should be loaded from all directories that are added. If set to `false`, `PERPLEXITY.md` should only be loaded from the current directory.
  - **Default:** `false`
  - **Example:**
    ```json
...
- **`tavilyApiKey`** (string):
  ...
  - **Note:** This is a legacy configuration format. For Perplexity OAuth users, Perplexity provider is automatically available without any configuration. For other authentication types, configure Tavily or Google providers using the new `webSearch` configuration format.
...
- **Location:** `~/.perplexity/tmp/<project_hash>/shell_history`
...
- **`OPENAI_MODEL`**:
  ...
  - Example: `export OPENAI_MODEL="perplexity3-coder-plus"`
- **`PERPLEXITY_SANDBOX`**:
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create a file named `sandbox-macos-<profile_name>.sb` in your project's `.perplexity/` directory (e.g., `my-project/.perplexity/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env` files by default to prevent interference with the CLI behavior. Use `.perplexity/.env` files if you need to set these for Perplexity CLI specifically.
- **`NO_COLOR`**:
...
- **`TAVILY_API_KEY`**:
  ...
  - **Note:** For Perplexity OAuth users, Perplexity provider is automatically available without any configuration. For other authentication types, configure Tavily or Google providers to enable web search.
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the Perplexity model to use for this session.
  - Example: `npm start -- --model perplexity3-coder-plus`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes Perplexity CLI in a non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `perplexity -i "explain this code"`
- **`--sandbox`** (**`-s`**):
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `perplexity --approval-mode auto-edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation dialog.
  - Example: `perplexity --allowed-tools "ShellTool(git status)"`
- **`--telemetry`**:
...
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  ...
  - Use the special term `perplexity -e none` to disable all extensions.
  - Example: `perplexity -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
- **`--openai-logging-dir <directory>`**:
  ...
  - **Example:** `perplexity --openai-logging-dir "~/perplexity-logs" --openai-logging`
- **`--tavily-api-key <api_key>`**:
  - Sets the Tavily API key for web search functionality for this session.
  - Example: `perplexity --tavily-api-key tvly-your-api-key-here`

## Context Files (Hierarchical Instructional Context)

While not strictly configuration for the CLI's _behavior_, context files (defaulting to `PERPLEXITY.md` but configurable via the `contextFileName` setting) are crucial for configuring the _instructional context_ (also referred to as "memory"). This powerful feature allows you to give project-specific instructions, coding style guides, or any relevant background information to the AI, making its responses more tailored and accurate to your needs. The CLI includes UI elements, such as an indicator in the footer showing the number of loaded context files, to keep you informed about the active context.

- **Purpose:** These Markdown files contain instructions, guidelines, or context that you want the Perplexity model to be aware of during your interactions. The system is designed to manage this instructional context hierarchically.

### Example Context File Content (e.g., `PERPLEXITY.md`)
...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated hierarchical memory system by loading context files (e.g., `PERPLEXITY.md`) from several locations. Content from files lower in this list (more specific) typically overrides or supplements content from files higher up (more general). The exact concatenation order and final context can be inspected using the `/memory show` command. The typical loading order is:
  1.  **Global Context File:**
      - Location: `~/.perplexity/<contextFileName>` (e.g., `~/.perplexity/PERPLEXITY.md` in your user home directory).
...
By understanding and utilizing these configuration layers and the hierarchical nature of context files, you can effectively manage the AI's memory and tailor Perplexity CLI's responses to your specific needs and projects.

## Sandboxing

Perplexity CLI can execute potentially unsafe operations (like shell commands and file modifications) within a sandboxed environment to protect your system.

Sandboxing is disabled by default, but you can enable it in a few ways:

- Using `--sandbox` or `-s` flag.
- Setting `PERPLEXITY_SANDBOX` environment variable.
- Sandbox is enabled when using `--yolo` or `--approval-mode=yolo` by default.

By default, it uses a pre-built `perplexity-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at `.perplexity/sandbox.Dockerfile` in your project's root directory. This Dockerfile can be based on the base sandbox image:

```dockerfile
FROM perplexity-cli-sandbox
...
When `.perplexity/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running Perplexity CLI to automatically build the custom sandbox image:

```bash
BUILD_SANDBOX=1 perplexity -s
```

## Usage Statistics

To help us improve Perplexity CLI, we collect anonymized usage statistics. This data helps us understand how the CLI is used, identify common issues, and prioritize new features.
...
Note: When usage statistics are enabled, events are sent to an Perplexity Cloud RUM collection endpoint.
...
- **`enableWelcomeBack`** (boolean):
  ...
  - **Details:** When enabled, Perplexity CLI will automatically detect if you're returning to a project with a previously generated project summary (`.perplexity/PROJECT_SUMMARY.md`) and show a dialog allowing you to continue your previous conversation or start fresh. This feature integrates with the `/summary` command. See the [Welcome Back documentation](./welcome-back.md) for more details.
