...
This monorepo contains two main packages: `@perplexity-cli/perplexity-cli` and `@perplexity-cli/perplexity-cli-core`.

## `@perplexity-cli/perplexity-cli`

This is the main package for Perplexity CLI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@perplexity-cli/perplexity-cli-core`. This means that whether a user installs the package with `npm install -g @perplexity-cli/perplexity-cli` or runs it directly with `npx @perplexity-cli/perplexity-cli`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/NoQuli/perplexity-cli/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/NoQuli/perplexity-cli/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
```bash
npm install -g @perplexity-cli/perplexity-cli@nightly
```
...
After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/NoQuli/perplexity-cli/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/NoQuli/perplexity-cli/pulls) of the repository.
...
- `npx -y @perplexity-cli/perplexity-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @perplexity-cli/perplexity-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @perplexity-cli/perplexity-cli && npm uninstall -g @perplexity-cli/perplexity-cli && npm cache clean --force &&  npm install @perplexity-cli/perplexity-cli@<version>`
...
1.  Go to the [Actions tab](https://github.com/NoQuli/perplexity-cli/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/perplexity-cli-0.1.6.tgz`).
...
1.  The `package.json` is Transformed:
    ...
    - Why: The final package.json must be different from the one used in development. Key changes include:
      ...
      - Removing workspace-specific "dependencies": { "@perplexity-cli/core": "workspace:\*" } and ensuring the core code is
...
2.  The JavaScript Bundle is Created:
    ...
    - File movement: packages/cli/dist/index.js + packages/core/dist/index.js -> (bundled by esbuild) -> `bundle`/perplexity.js (or a
      similar name).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @perplexity-cli/perplexity-cli`.
