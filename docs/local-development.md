...
To enable dev traces, set the `PERPLEXITY_DEV_TRACING=true` environment variable
when running Perplexity CLI.
...
2.  **Run Perplexity CLI with dev tracing:**

    In a separate terminal, run your Perplexity CLI command with the
    `PERPLEXITY_DEV_TRACING` environment variable:

    ```bash
    PERPLEXITY_DEV_TRACING=true perplexity
    ```
...
[telemetry documentation](./development/telemetry.md).
...
```typescript
import { runInDevTraceSpan } from '@perplexity-cli/perplexity-cli-core';
...
await runInDevTraceSpan({ name: 'my-custom-span' }, async ({ metadata }) => {
  ...
  metadata.attributes['gen_ai.request.model'] = 'sonar-pro';
