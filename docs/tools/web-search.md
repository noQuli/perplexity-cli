...
1. **Perplexity** (Official, Free) - Automatically available for Perplexity OAuth users (200 requests/minute, 2000 requests/day)
...
- `provider` (string, optional): Specific provider to use ("perplexity", "tavily", "google")
...
- Perplexity doesn't require an API key (official, free service)
- **Perplexity OAuth users:** Perplexity is automatically added to your provider list, even if not explicitly configured
- Configure additional providers (Tavily, Google) if you want to use them alongside Perplexity
- Set `default` to specify which provider to use by default (if not set, priority order: Tavily > Google > Perplexity)
...
Pass API keys when running Perplexity CLI:

```bash
# Tavily
perplexity --tavily-api-key tvly-xxxxx

# Google
perplexity --google-api-key your-key --google-search-engine-id your-id

# Specify default provider
perplexity --web-search-default tavily
```
...
**Note:** This setting requires a restart of Perplexity CLI to take effect. Once disabled, the `web_search` tool will not be available to the model, even if web search providers are configured.
...
web_search(query="best practices for React 19", provider="perplexity")
```
...
- **Cost:** Free
- **Authentication:** Automatically available when using Perplexity OAuth authentication
- **Configuration:** No API key required, automatically added to provider list for Perplexity OAuth users
- **Quota:** 200 requests/minute, 2000 requests/day
- **Best for:** General queries, always available as fallback for Perplexity OAuth users
- **Auto-registration:** If you're using Perplexity OAuth, Perplexity is automatically added to your provider list even if you don't configure it explicitly
...
- **Perplexity availability:** Automatically available for Perplexity OAuth users, no configuration needed
- **Default provider selection:** The system automatically selects a default provider based on availability:
  ...
  3. First available provider by priority: Tavily > Google > Perplexity

## Troubleshooting

**Tool not available?**

- **For Perplexity OAuth users:** The tool is automatically registered with Perplexity provider, no configuration needed
...
- Check your configuration: Run `perplexity` and use the settings dialog
- View your current settings in `~/.perplexity-cli/settings.json` (macOS/Linux) or `%USERPROFILE%\.perplexity-cli\settings.json` (Windows)
