...
This document describes the `todo_write` tool for Perplexity CLI.
...
## How to use `todo_write` with Perplexity CLI
...
The tool stores todo lists in your home directory (`~/.perplexity/todos/`) with session-specific files, so each coding session maintains its own task list.
