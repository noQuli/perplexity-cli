# MCP servers with Perplexity CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Perplexity CLI.
...
Perplexity CLI integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the Perplexity API
...
Perplexity CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
Perplexity CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.perplexity/mcp-oauth-tokens.json`
...
When Perplexity CLI starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet Perplexity API requirements:
...
Once discovered, MCP tools are available to the Perplexity model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for Perplexity API compatibility
...
When Perplexity CLI receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the Perplexity model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Perplexity CLI. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Perplexity CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Perplexity CLI" reverent
```
...
## Managing MCP Servers with `perplexity mcp`
...
### Adding a Server (`perplexity mcp add`)

The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.perplexity/settings.json` or the project config `.perplexity/settings.json` file.

**Command:**

```bash
perplexity mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
perplexity mcp add <name> <command> [args...]

# Example: Adding a local server
perplexity mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
perplexity mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
perplexity mcp add --transport http <name> <url>

# Example: Adding an HTTP server
perplexity mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
perplexity mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
perplexity mcp add --transport sse <name> <url>

# Example: Adding an SSE server
perplexity mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
perplexity mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"
```

### Listing Servers (`perplexity mcp list`)
...
### Removing a Server (`perplexity mcp remove`)
...
```bash
perplexity mcp remove <name>
```

**Example:**

```bash
perplexity mcp remove my-server
```
