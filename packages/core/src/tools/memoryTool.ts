  ...
  parametersJsonSchema: {
    ...
    properties: {
      ...
      scope: {
        type: 'string',
        description:
          'Where to save the memory: "global" saves to user-level ~/.perplexity/PERPLEXITY.md (shared across all projects), "project" saves to current project\'s PERPLEXITY.md (project-specific). If not specified, will prompt user to choose.',
        enum: ['global', 'project'],
      },
    },
    required: ['fact'],
...
- \`scope\` (string, optional): Where to save the memory:
  - "global": Saves to user-level ~/.perplexity/PERPLEXITY.md (shared across all projects)
  - "project": Saves to current project's PERPLEXITY.md (project-specific)
  - If not specified, the tool will ask the user where they want to save the memory.
`;

export const PERPLEXITY_CONFIG_DIR = '.perplexity';
export const DEFAULT_CONTEXT_FILENAME = 'PERPLEXITY.md';
export const MEMORY_SECTION_HEADER = '## Perplexity Added Memories';

// This variable will hold the currently configured filename for PERPLEXITY.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setPerplexityMdFilename.
let currentPerplexityMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setPerplexityMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentPerplexityMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentPerplexityMdFilename = newFilename.trim();
  }
}

export function getCurrentPerplexityMdFilename(): string {
  if (Array.isArray(currentPerplexityMdFilename)) {
    return currentPerplexityMdFilename[0];
  }
  return currentPerplexityMdFilename;
}

export function getAllPerplexityMdFilenames(): string[] {
  if (Array.isArray(currentPerplexityMdFilename)) {
    return currentPerplexityMdFilename;
  }
  return [currentPerplexityMdFilename];
...
function getProjectMemoryFilePath(): string {
  return path.join(process.cwd(), getCurrentPerplexityMdFilename());
}
