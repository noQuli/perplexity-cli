...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .perplexityignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_perplexity_ignore?: boolean;
  };
}
 ...
 * This combines the shared patterns with dynamic patterns like PERPLEXITY.md.
...
${this.config.getTargetDir()}
    ...
    const perplexityIgnorePatterns = this.config
      .getFileService()
      .getPerplexityIgnorePatterns();
    const finalExclusionPatternsForDescription: string[] =
      paramUseDefaultExcludes
        ? [
            ...getDefaultExcludes(this.config),
            ...paramExcludes,
            ...perplexityIgnorePatterns,
          ]
        : [...paramExcludes, ...perplexityIgnorePatterns];
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .perplexityignore patterns contributed to the final list of exclusions
    if (perplexityIgnorePatterns.length > 0) {
      const perplexityPatternsInEffect = perplexityIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (perplexityPatternsInEffect > 0) {
        excludeDesc += ` (includes ${perplexityPatternsInEffect} from .perplexityignore)`;
      }
    }
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      const { filteredPaths, gitIgnoredCount, perplexityIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativeEntries, {
          ...
          respectPerplexityIgnore:
            this.params.file_filtering_options?.respect_perplexity_ignore ??
            this.config.getFileFilteringOptions().respectPerplexityIgnore ??
            DEFAULT_FILE_FILTERING_OPTIONS.respectPerplexityIgnore,
        });
      ...
      // Add info about perplexity-ignored files if any were filtered
      if (perplexityIgnoredCount > 0) {
        skippedFiles.push({
          path: `${perplexityIgnoredCount} file(s)`,
          reason: 'perplexity ignored',
        });
      }
    } catch (error) {
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .perplexityignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_perplexity_ignore: {
              description:
                'Optional: Whether to respect .perplexityignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
