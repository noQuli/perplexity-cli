...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .perplexityignore
    const filteringOptions = this.getFileFilteringOptions();
    if (!filteringOptions.respectGitIgnore) {
      rgArgs.push('--no-ignore-vcs');
    }

    if (filteringOptions.respectPerplexityIgnore) {
      const perplexityIgnorePath = path.join(
        this.config.getTargetDir(),
        '.perplexityignore',
      );
      if (fs.existsSync(perplexityIgnorePath)) {
        rgArgs.push('--ignore-file', perplexityIgnorePath);
      }
    }
