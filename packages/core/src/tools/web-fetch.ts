...
> {
  ...
  private async executeDirectFetch(signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      const perplexityClient = this.config.getPerplexityClient();
...
---`;
      ...
      const result = await perplexityClient.generateContent(
        [{ role: 'user', parts: [{ text: fallbackPrompt }] }],
        {},
        signal,
        this.config.getModel() || DEFAULT_PERPLEXITY_MODEL,
      );
