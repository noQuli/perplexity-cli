...
// Get the mock control after mocking
const mockControl = await vi
  .importMock<
    typeof import('../__mocks__/fs/promises.js')
  >('../__mocks__/fs/promises.js')
  .then((m) => m.mockControl);
...
describe('ReadManyFilesTool', () => {
  ...
  beforeEach(async () => {
    ...
    fs.writeFileSync(path.join(tempRootDir, '.perplexityignore'), 'foo.*');
  ...
  describe('execute', () => {
    ...
    it('should return error if path is ignored by a .perplexityignore pattern', async () => {
      createFile('foo.bar', '');
      createFile('bar.ts', '');
      createFile('foo.quux', '');
