...
const MEMORY_SECTION_HEADER = '## Perplexity Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset PERPLEXITY_MD_FILENAME to its original value after each test
    setPerplexityMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setPerplexityMdFilename', () => {
    it('should update currentPerplexityMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setPerplexityMdFilename(newName);
      expect(getCurrentPerplexityMdFilename()).toBe(newName);
    });

    it('should not update currentPerplexityMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentPerplexityMdFilename(); // Get current before trying to change
      setPerplexityMdFilename('  ');
      expect(getCurrentPerplexityMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setPerplexityMdFilename(newNames);
      expect(getCurrentPerplexityMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllPerplexityMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success for global scope', async () => {
      ...
      // Use getCurrentPerplexityMdFilename for the default expectation before any setPerplexityMdFilename calls in a test
      const expectedFilePath = path.join(
        os.homedir(),
        '.perplexity',
        getCurrentPerplexityMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted for global scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.perplexity', 'PERPLEXITY.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(
          path.join('mock', 'home', '.perplexity'),
        );
        expect(result.fileName).toContain('PERPLEXITY.md');
        expect(result.fileDiff).toContain('Index: PERPLEXITY.md');
        expect(result.fileDiff).toContain('+## Perplexity Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Perplexity Added Memories');
    ...
    it('should return confirmation details when memory file is not allowlisted for project scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join(process.cwd(), 'PERPLEXITY.md');
        expect(result.title).toBe(
    ...
    it('should prompt for scope selection when scope is not specified', async () => {
      ...
      if (result && result.type === 'edit') {
        ...
        expect(result.fileName).toBe('PERPLEXITY.md');
        expect(result.fileDiff).toContain('Test fact');
        expect(result.fileDiff).toContain('--- PERPLEXITY.md');
        expect(result.fileDiff).toContain('+++ PERPLEXITY.md');
        expect(result.fileDiff).toContain('+- Test fact');
    ...
    it('should show correct file paths in scope selection prompt', async () => {
      ...
      if (result && result.type === 'edit') {
        const globalPath = path.join('~', '.perplexity', 'PERPLEXITY.md');
        const projectPath = path.join(process.cwd(), 'PERPLEXITY.md');
