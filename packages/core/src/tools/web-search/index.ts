...
> {
  ...
  private createProvider(config: WebSearchProviderConfig): WebSearchProvider {
    switch (config.type) {
      ...
      case 'perplexity': {
        // Pass auth type to Perplexity provider for availability check
        const authType = this.config.getAuthType();
        const perplexityConfig: PerplexityProviderConfig = {
          ...config,
          authType: authType as string | undefined,
        };
        return new PerplexityProvider(perplexityConfig);
      }
      default:
      ...
      Kind.Search,
      {
        ...
        properties: {
          ...
          provider: {
            type: 'string',
            description:
              'Optional provider to use for the search (e.g., "tavily", "google", "perplexity"). IMPORTANT: Only specify this parameter if you explicitly know which provider to use. Otherwise, omit this parameter entirely and let the system automatically select the appropriate provider based on availability and configuration. The system will choose the best available provider automatically.',
          },
        },
        required: ['query'],
      },
