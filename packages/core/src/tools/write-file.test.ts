...
const rootDir = path.resolve(os.tmpdir(), 'perplexity-code-test-root');
...
let mockPerplexityClientInstance: Mocked<PerplexityClient>;
const mockIdeClient = {
  openDiff: vi.fn(),
...
const mockConfigInternal = {
  ...
  getPerplexityClient: vi.fn(), // Initialize as a plain mock function
  ...
  setPerplexityMdFileCount: vi.fn(),
  getToolRegistry: () =>
    ({
      registerTool: vi.fn(),
      discoverTools: vi.fn(),
...
describe('WriteFileTool', () => {
  ...
  beforeEach(() => {
    ...
    // Setup PerplexityClient mock
    mockPerplexityClientInstance = new (vi.mocked(PerplexityClient))(
      mockConfig,
    ) as Mocked<PerplexityClient>;
    vi.mocked(PerplexityClient).mockImplementation(
      () => mockPerplexityClientInstance,
    );

    // Now that mockPerplexityClientInstance is initialized, set the mock implementation for getPerplexityClient
    mockConfigInternal.getPerplexityClient.mockReturnValue(
      mockPerplexityClientInstance,
    );
