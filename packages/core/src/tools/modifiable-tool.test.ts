...
describe('modifyWithEditor', () => {
  ...
  afterEach(async () => {
    ...
    const diffDir = path.join(os.tmpdir(), 'perplexity-code-tool-modify-diffs');
  ...
  describe('successful modification', () => {
    ...
    it('should create temp directory if it does not exist', async () => {
      const diffDir = path.join(
        os.tmpdir(),
        'perplexity-code-tool-modify-diffs',
      );
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(oldFilePath).toMatch(
      /perplexity-code-modify-test-file-old-\d+\.txt$/,
    );
    expect(newFilePath).toMatch(
      /perplexity-code-modify-test-file-new-\d+\.txt$/,
    );
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/perplexity-code-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/perplexity-code-modify-test-file-new-\d+$/);
