...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'perplexity-code-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `perplexity-code-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `perplexity-code-modify-${fileName}-new-${timestamp}${ext}`,
  );
