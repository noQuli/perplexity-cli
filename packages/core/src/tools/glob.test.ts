...
import { partListUnionToString } from '../core/perplexityRequest.js';
...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .perplexityignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.perplexityignore'),
        '*.perplexityignored.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'a.perplexityignored.txt'),
        'ignored content',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'b.notignored.txt'),
      ...
      // Recreate the tool to pick up the new .perplexityignore file
      globTool = new GlobTool(mockConfig);
      ...
      expect(result.llmContent).toContain('Found 3 file(s)'); // fileA.txt, FileB.TXT, b.notignored.txt
      expect(result.llmContent).not.toContain('a.perplexityignored.txt');
    });
  });
