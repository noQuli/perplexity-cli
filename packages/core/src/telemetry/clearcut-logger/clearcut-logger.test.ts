...
expect.extend({
  ...
  toHaveMetadataValue(
    ...
    const data = metadata.find((m) => m.perplexity_cli_key === key)?.value;
  ...
  toHaveMetadataKey(received: LogEventEntry[], key: EventMetadataKey) {
    ...
    const pass = metadata.some((m) => m.perplexity_cli_key === key);
...
describe('ClearcutLogger', () => {
  ...
  describe('createLogEvent', () => {
    ...
    it('logs default metadata', () => {
      ...
      const auth_type = AuthType.USE_PERPLEXITY;
  ...
  describe('enqueueLogEvent', () => {
    ...
    it('should evict the oldest event when the queue is full', () => {
      ...
      for (let i = 0; i < TEST_ONLY.MAX_EVENTS; i++) {
        logger!.enqueueLogEvent(
          logger!.createLogEvent(EventNames.API_ERROR, [
            {
              perplexity_cli_key:
                EventMetadataKey.PERPLEXITY_CLI_AI_ADDED_LINES,
              value: `${i}`,
            },
          ]),
        );
      ...
      logger!.enqueueLogEvent(
        logger!.createLogEvent(EventNames.API_ERROR, [
          {
            perplexity_cli_key: EventMetadataKey.PERPLEXITY_CLI_AI_ADDED_LINES,
  ...
  describe('logToolCallEvent', () => {
    it('logs an event with all diff metadata', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_AI_REMOVED_LINES,
        '2',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_AI_ADDED_CHARS,
        '3',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_AI_REMOVED_CHARS,
        '4',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_USER_ADDED_LINES,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_USER_REMOVED_LINES,
        '6',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_USER_ADDED_CHARS,
        '7',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PERPLEXITY_CLI_USER_REMOVED_CHARS,
        '8',
      ]);
    });
