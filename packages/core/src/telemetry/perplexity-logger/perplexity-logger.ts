...
export class PerplexityLogger {
  private static instance: PerplexityLogger;
  ...
  static getInstance(config?: Config): PerplexityLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!PerplexityLogger.instance) {
      PerplexityLogger.instance = new PerplexityLogger(config);
    }

    return PerplexityLogger.instance;
  }
  ...
  enqueueLogEvent(event: RumEvent): void {
    try {
      ...
      if (wasAtCapacity && this.config?.getDebugMode()) {
        console.debug(
          `PerplexityLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      if (this.config?.getDebugMode()) {
        console.error('PerplexityLogger: Failed to enqueue log event.', error);
      }
    }
  }
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    if (numEventsToRequeue === 0) {
      if (this.config?.getDebugMode()) {
        console.debug(
          `PerplexityLogger: No events re-queued (queue size: ${this.events.size})`,
        );
      }
      return;
    }
    ...
    if (this.config?.getDebugMode()) {
      console.debug(
        `PerplexityLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
      );
    }
  }
}
