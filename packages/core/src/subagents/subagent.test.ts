...
import { PerplexityChat } from '../core/perplexityChat.js';
...
vi.mock('../core/perplexityChat.js');
...
describe('subagent.ts', () => {
  ...
  describe('SubAgentScope', () => {
    ...
    beforeEach(async () => {
      ...
      vi.mocked(PerplexityChat).mockImplementation(
        () =>
          ({
    ...
    const getGenerationConfigFromMock = (
      ...
      const callArgs = vi.mocked(PerplexityChat).mock.calls[callIndex];
    ...
    describe('runNonInteractive - Initialization and Prompting', () => {
      it('should correctly template the system prompt and initialize PerplexityChat', async () => {
        const { config } = await createMockConfig();

        vi.mocked(PerplexityChat).mockClear();
        ...
        // Check if PerplexityChat was initialized correctly by the subagent
        expect(PerplexityChat).toHaveBeenCalledTimes(1);
        const callArgs = vi.mocked(PerplexityChat).mock.calls[0];
