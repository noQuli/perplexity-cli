...
import type { ServerPerplexityStreamEvent } from '../core/turn.js';
import { PerplexityEventType } from '../core/turn.js';
import {
  logLoopDetected,
...
export class LoopDetectionService {
   ...
   */
  addAndCheck(event: ServerPerplexityStreamEvent): boolean {
  ...
  private async checkForLoopWithLLM(signal: AbortSignal) {
    ...
    try {
      result = await this.config.getBaseLlmClient().generateJson({
        contents,
        schema,
        model: this.config.getModel() || DEFAULT_PERPLEXITY_MODEL,
