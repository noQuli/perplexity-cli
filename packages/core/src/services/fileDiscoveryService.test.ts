...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .perplexityignore patterns even when not in a git repo', async () => {
      await createTestFile('.perplexityignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldPerplexityIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldPerplexityIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.perplexityignore', 'logs/');
    });

    it('should filter out git-ignored and perplexity-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectPerplexityIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectPerplexityIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldPerplexityIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.perplexityignore', '*.log');
    });
    ...
    it('should return true for perplexity-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldPerplexityIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-perplexity-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldPerplexityIgnoreFile(
