...
export class IdeClient {
  ...
  async connect(options: { logToConsole?: boolean } = {}): Promise<void> {
    ...
    if (!this.currentIde) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run Perplexity Code in one of these supported IDEs: VS Code or VS Code forks`,
        false,
      );
      return;
    }
    ...
    this.authToken =
      ...
      process.env['PERPLEXITY_CODE_IDE_AUTH_TOKEN'];

    const workspacePath =
      this.connectionConfig?.workspacePath ??
      process.env['PERPLEXITY_CODE_IDE_WORKSPACE_PATH'];
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. Perplexity Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['PERPLEXITY_CODE_IDE_SERVER_PORT'];
    if (!port) {
      return undefined;
    }
    return port;
  ...
  private getStdioConfigFromEnv(): StdioConfig | undefined {
    const command = process.env['PERPLEXITY_CODE_IDE_SERVER_STDIO_COMMAND'];
    if (!command) {
      return undefined;
    }

    const argsStr = process.env['PERPLEXITY_CODE_IDE_SERVER_STDIO_ARGS'];
    let args: string[] = [];
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          logger.error(
            'PERPLEXITY_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        logger.error(
          'Failed to parse PERPLEXITY_CODE_IDE_SERVER_STDIO_ARGS:',
          e,
        );
      }
    }
  ...
  > {
    ...
    try {
      const portFile = path.join(
        os.tmpdir(),
        `perplexity-code-ide-server-${this.ideProcessInfo.pid}.json`,
      );
      const portFileContents = await fs.promises.readFile(portFile, 'utf8');
      return JSON.parse(portFileContents);
    } catch (_) {
      // For newer extension versions, the file name matches the pattern
      // /^perplexity-code-ide-server-${pid}-\d+\.json$/. If multiple IDE
      // windows are open, multiple files matching the pattern are expected to
      // exist.
    }

    const portFileDir = path.join(os.tmpdir(), 'perplexity', 'ide');
    let portFiles;
    try {
    ...
    const fileRegex = new RegExp(
      `^perplexity-code-ide-server-${this.ideProcessInfo.pid}-\\d+\\.json$`,
    );
