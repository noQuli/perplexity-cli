...
export async function handleFallback(
  ...
  if (authType === AuthType.PERPLEXITY_OAUTH) {
    return handlePerplexityOAuthError(error);
  }

  // Applicability Checks
  if (authType !== AuthType.LOGIN_WITH_GOOGLE) return null;

  const fallbackModel = DEFAULT_PERPLEXITY_FLASH_MODEL;
...
/**
 * Handles Perplexity OAuth authentication errors and rate limiting
 */
async function handlePerplexityOAuthError(
  error?: unknown,
): Promise<string | null> {
  if (!error) {
    return null;
  ...
  if (isAuthError) {
    console.warn(
      'Perplexity OAuth authentication error detected:',
      errorMessage,
    );
    // The PerplexityContentGenerator should automatically handle token refresh
    // If it still fails, it likely means the refresh token is also expired
    console.log(
      'Note: If this persists, you may need to re-authenticate with Perplexity OAuth',
    );
    return null;
  }

  if (isRateLimitError) {
    console.warn('Perplexity API rate limit encountered:', errorMessage);
