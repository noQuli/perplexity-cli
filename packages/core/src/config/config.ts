...
import { PerplexityClient } from '../core/client.js';
import {
  AuthType,
...
import { MemoryTool, setPerplexityMdFilename } from '../tools/memoryTool.js';
...
import { DEFAULT_PERPLEXITY_BASE_URL } from '../core/openaiContentGenerator/constants.js';
...
export interface PerplexityCLIExtension {
  name: string;
  version: string;
  isActive: boolean;
  path: string;
...
export interface ConfigParameters {
  ...
  extensions?: PerplexityCLIExtension[];
...
export class Config {
  ...
  private perplexityMdFileCount: number;
  ...
  private readonly _extensions: PerplexityCLIExtension[];
  ...
  constructor(params: ConfigParameters) {
    ...
    this.embeddingModel =
      params.embeddingModel ?? DEFAULT_PERPLEXITY_EMBEDDING_MODEL;
    ...
    this.perplexityMdFileCount = params.perplexityMdFileCount ?? 0;
    ...
    this.gitCoAuthor = {
      ...
      name: params.gitCoAuthor?.name ?? 'Perplexity-Coder',
      email: params.gitCoAuthor?.email ?? 'perplexity-coder@perplexity.ai',
    };
    ...
    this.fileFiltering = {
      ...
      respectPerplexityIgnore:
        params.fileFiltering?.respectPerplexityIgnore ?? true,
    ...
    this._generationConfig = {
      ...
      baseUrl: params.generationConfig?.baseUrl || DEFAULT_PERPLEXITY_BASE_URL,
    };
    ...
    if (params.contextFileName) {
      setPerplexityMdFilename(params.contextFileName);
    }
    ...
    this.perplexityClient = new PerplexityClient(this);
  ...
  async refreshAuth(authMethod: AuthType, _isInitialAuth?: boolean) {
    // Vertex and Genai have incompatible encryption and sending history with
    // throughtSignature from Genai to Vertex will fail, we need to strip them
    if (
      this.contentGeneratorConfig?.authType === AuthType.USE_PERPLEXITY &&
      authMethod === AuthType.LOGIN_WITH_GOOGLE
    ) {
      // Restore the conversation history to the new client
      this.perplexityClient.stripThoughtsFromHistory();
    }
  ...
  getModel(): string {
    return this.contentGeneratorConfig?.model || DEFAULT_PERPLEXITY_MODEL;
  }
  ...
  getPerplexityMdFileCount(): number {
    return this.perplexityMdFileCount;
  }

  setPerplexityMdFileCount(count: number): void {
    this.perplexityMdFileCount = count;
  }
  ...
  getPerplexityClient(): PerplexityClient {
  ...
  getFileFilteringRespectPerplexityIgnore(): boolean {
    return this.fileFiltering.respectPerplexityIgnore;
  }

  getFileFilteringOptions(): FileFilteringOptions {
    return {
      respectGitIgnore: this.fileFiltering.respectGitIgnore,
      respectPerplexityIgnore: this.fileFiltering.respectPerplexityIgnore,
    };
  }
  ...
  getExtensions(): PerplexityCLIExtension[] {
  ...
  async createToolRegistry(
    ...
    // buildWebSearchConfig ensures perplexity-oauth users get perplexity provider, so
