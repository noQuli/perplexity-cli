...
import { setPerplexityMdFilename as mockSetPerplexityMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryTool', () => ({
  ...
  setPerplexityMdFilename: vi.fn(),
  getCurrentPerplexityMdFilename: vi.fn(() => 'PERPLEXITY.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'PERPLEXITY.md',
  PERPLEXITY_CONFIG_DIR: '.perplexity',
}));
...
describe('Server Config (config.ts)', () => {
  const MODEL = 'perplexity3-coder-plus';
  ...
  const EMBEDDING_MODEL = 'perplexity-embedding';
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      const authType = AuthType.USE_PERPLEXITY;
      ...
      expect(PerplexityClient).toHaveBeenCalledWith(config);
    ...
    it('should strip thoughts when switching from GenAI to Vertex', async () => {
      ...
      await config.refreshAuth(AuthType.USE_PERPLEXITY);

      await config.refreshAuth(AuthType.LOGIN_WITH_GOOGLE);

      expect(
        config.getPerplexityClient().stripThoughtsFromHistory,
  ...
  it('Config constructor should call setPerplexityMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetPerplexityMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setPerplexityMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetPerplexityMdFilename).not.toHaveBeenCalled();
  });
  ...
  describe('Usage Statistics', () => {
    ...
    it('logs the session start event', async () => {
      ...
      expect(
        PerplexityLogger.prototype.logStartSessionEvent,
...
describe('BaseLlmClient Lifecycle', () => {
  const MODEL = 'perplexity-pro';
  const SANDBOX: SandboxConfig = {
    command: 'docker',
    image: 'perplexity-cli-sandbox',
  };
  const TARGET_DIR = '/path/to/target';
  const DEBUG_MODE = false;
  const QUESTION = 'test question';
  ...
  it('should successfully initialize BaseLlmClient after refreshAuth is called', async () => {
    const config = new Config(baseParams);
    const authType = AuthType.USE_PERPLEXITY;
    const mockContentConfig = { model: 'perplexity-flash', apiKey: 'test-key' };
