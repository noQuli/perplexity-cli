...
export class Logger {
  private perplexityDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.perplexityDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.perplexityDir, LOG_FILE_NAME);
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.perplexityDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.perplexityDir) {
      console.error(
