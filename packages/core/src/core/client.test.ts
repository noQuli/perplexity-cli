...
vi.mock('../telemetry/index.js', async (importOriginal) => {
  ...
  return {
    ...
    // but we can spy on PerplexityLogger if needed
  };
});
...
describe('isThinkingSupported', () => {
  it('should return true for sonar-reasoning-pro', () => {
    expect(isThinkingSupported('sonar-reasoning-pro')).toBe(true);
  });

  it('should return false for other models', () => {
    expect(isThinkingSupported('sonar')).toBe(false);
    expect(isThinkingSupported('sonar-pro')).toBe(false);
    expect(isThinkingSupported('some-other-model')).toBe(false);
  });
});

describe('isThinkingDefault', () => {
  it('should return false for sonar', () => {
    expect(isThinkingDefault('sonar')).toBe(false);
  });

  it('should return true for sonar-reasoning-pro', () => {
    expect(isThinkingDefault('sonar-reasoning-pro')).toBe(true);
  });

  it('should return false for other models', () => {
    expect(isThinkingDefault('sonar-pro')).toBe(false);
    expect(isThinkingDefault('some-other-model')).toBe(false);
  });
});

describe('Perplexity Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the PerplexityClient constructor kicks off an async process (startChat)
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectDir: vi
          .fn()
          .mockReturnValue(
            '/test/project/root/.perplexity/projects/test-project',
          ),
      },
    ...
    vi.mocked(mockConfig.getPerplexityClient).mockReturnValue(client);
  });
  ...
  describe('tryCompressChat', () => {
    ...
    function setup({
      ...
      const mockOriginalChat: Partial<PerplexityChat> = {
        getHistory: vi.fn((_curated?: boolean) => chatHistory),
        setHistory: vi.fn(),
        stripThoughtsFromHistory: vi.fn(),
      };
      client['chat'] = mockOriginalChat as PerplexityChat;
      ...
      const mockNewChat: Partial<PerplexityChat> = {
      ...
      client['startChat'] = vi
        .fn()
        .mockResolvedValue(mockNewChat as PerplexityChat);
...
\`\`\`
    ...
    it('should not add context if ideMode is enabled but no open files', async () => {
      ...
      const mockChat: Partial<PerplexityChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
        stripThoughtsFromHistory: vi.fn(),
      };
      client['chat'] = mockChat as PerplexityChat;
...
\`\`\`
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: PerplexityEventType.MaxSessionTurns }]);
    ...
    describe('IDE context with pending tool calls', () => {
      let mockChat: Partial<PerplexityChat>;
    ...
    it('should not call checkNextSpeaker when turn.run() yields a value then an error', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: PerplexityEventType.Content, value: 'some content' };
        yield {
          type: PerplexityEventType.Error,
          value: { error: { message: 'test error' } },
        };
  ...
  describe('generateContent', () => {
    it('should call generateContent with the correct parameters', async () => {
      ...
      expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(
        {
          model: DEFAULT_PERPLEXITY_FLASH_MODEL,
          config: {
            abortSignal,
    ...
    it('should use the Flash model when fallback mode is active', async () => {
      ...
      const generationConfig = { temperature: 0.5 };
      const abortSignal = new AbortController().signal;
      const requestedModel = 'perplexity-2.5-pro'; // A non-flash model
