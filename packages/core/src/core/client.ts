...
import { PerplexityChat } from './perplexityChat.js';
import {
  getCoreSystemPrompt,
...
import {
  ...
  type ServerPerplexityStreamEvent,
} from './turn.js';
...
export function isThinkingSupported(model: string) {
  // Perplexity sonar-reasoning models support thinking
  return model.startsWith('sonar-reasoning');
}

export function isThinkingDefault(model: string) {
  // Reasoning is always enabled for sonar-reasoning models
  return model.startsWith('sonar-reasoning');
}
...
export class PerplexityClient {
  ...
  async startChat(extraHistory?: Content[]): Promise<PerplexityChat> {
    this.forceFullIdeContext = true;
  ...
  async *sendMessageStream(
  ...
  ): AsyncGenerator<ServerPerplexityStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: PerplexityEventType.MaxSessionTurns };
    ...
    if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
      yield { type: PerplexityEventType.ChatCompressed, value: compressed };
    }
    ...
    // waiting for a response. The Perplexity API requires that a functionResponse
    ...
    if (!this.config.getSkipLoopDetection()) {
      ...
      if (loopDetected) {
        yield { type: PerplexityEventType.LoopDetected };
        return turn;
      }
    }
    ...
    for await (const event of resultStream) {
      ...
      yield event;
      if (event.type === PerplexityEventType.Error) {
        return turn;
      }
    }
