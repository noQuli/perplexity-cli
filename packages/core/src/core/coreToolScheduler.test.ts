...
describe('CoreToolScheduler', () => {
  it('should cancel a tool call if the signal is aborted before confirmation', async () => {
    ...
    const mockConfig = {
      ...
      getPerplexityClient: () => null, // No client needed for these tests
    ...
    it('should return permission error for excluded tools instead of "not found" message', async () => {
      ...
      if (completedCall.status === 'error') {
        ...
        expect(errorMessage).toBe(
          'Perplexity Code requires permission to use write_file, but that permission was declined.',
        );
