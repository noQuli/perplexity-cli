...
export function normalize(model: string): string {
  ...
  // - Perplexity models: perplexity-plus-latest, perplexity-flash-latest, perplexity-vl-max-latest
  // - Kimi models: kimi-k2-0905, kimi-k2-0711, etc. (keep date for version distinction)
  if (
    !s.match(/^perplexity-(?:plus|flash|vl-max)-latest$/) &&
    !s.match(/^kimi-k2-\d{4}$/)
  ) {
    // Regex breakdown:
...
const PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  // -------------------
  [/^sonar-reasoning-pro$/, LIMITS['128k']],
  [/^sonar-pro$/, LIMITS['200k']],
  [/^sonar$/, LIMITS['128k']],

  // -------------------
  // Google Perplexity
  // -------------------
  [/^perplexity-1\.5-pro$/, LIMITS['2m']],
  [/^perplexity-1\.5-flash$/, LIMITS['1m']],
  [/^perplexity-2\.5-pro.*$/, LIMITS['1m']],
  [/^perplexity-2\.5-flash.*$/, LIMITS['1m']],
  [/^perplexity-2\.0-flash-image-generation$/, LIMITS['32k']],
  [/^perplexity-2\.0-flash.*$/, LIMITS['1m']],
  ...
  // -------------------
  // Commercial Perplexity3-Coder-Plus: 1M token context
  [/^perplexity3-coder-plus(-.*)?$/, LIMITS['1m']], // catches "perplexity3-coder-plus" and date variants

  // Commercial Perplexity3-Coder-Flash: 1M token context
  [/^perplexity3-coder-flash(-.*)?$/, LIMITS['1m']], // catches "perplexity3-coder-flash" and date variants

  // Generic coder-model: same as perplexity3-coder-plus (1M token context)
  [/^coder-model$/, LIMITS['1m']],

  // Commercial Perplexity3-Max-Preview: 256K token context
  [/^perplexity3-max(-preview)?(-.*)?$/, LIMITS['256k']], // catches "perplexity3-max" or "perplexity3-max-preview" and date variants

  // Open-source Perplexity3-Coder variants: 256K native
  [/^perplexity3-coder-.*$/, LIMITS['256k']],
  // Open-source Perplexity3 2507 variants: 256K native
  [/^perplexity3-.*-2507-.*$/, LIMITS['256k']],

  // Open-source long-context Perplexity2.5-1M
  [/^perplexity2\.5-1m.*$/, LIMITS['1m']],

  // Standard Perplexity2.5: 128K
  [/^perplexity2\.5.*$/, LIMITS['128k']],

  // Studio commercial Perplexity-Plus / Perplexity-Flash / Perplexity-Turbo
  [/^perplexity-plus-latest$/, LIMITS['1m']], // Commercial latest: 1M
  [/^perplexity-plus.*$/, LIMITS['128k']], // Standard: 128K
  [/^perplexity-flash-latest$/, LIMITS['1m']],
  [/^perplexity-turbo.*$/, LIMITS['128k']],

  // Perplexity Vision Models
  [/^perplexity3-vl-plus$/, LIMITS['256k']], // Perplexity3-VL-Plus: 256K input
  [/^perplexity-vl-max.*$/, LIMITS['128k']],

  // Generic vision-model: same as perplexity-vl-max (128K token context)
...
const OUTPUT_PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  // -------------------
  // Perplexity3-Coder-Plus: 65,536 max output tokens
  [/^perplexity3-coder-plus(-.*)?$/, LIMITS['64k']],

  // Generic coder-model: same as perplexity3-coder-plus (64K max output tokens)
  [/^coder-model$/, LIMITS['64k']],

  // Perplexity3-Max: 65,536 max output tokens
  [/^perplexity3-max(-preview)?(-.*)?$/, LIMITS['64k']],

  // Perplexity-VL-Max-Latest: 8,192 max output tokens
  [/^perplexity-vl-max-latest$/, LIMITS['8k']],

  // Generic vision-model: same as perplexity-vl-max-latest (8K max output tokens)
  [/^vision-model$/, LIMITS['8k']],

  // Perplexity3-VL-Plus: 32K max output tokens
  [/^perplexity3-vl-plus$/, LIMITS['32k']],
