...
import { PERPLEXITY_CONFIG_DIR } from '../tools/memoryTool.js';
...
describe('Core System Prompt (prompts.ts)', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('PERPLEXITY_SYSTEM_MD', undefined);
    vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', undefined);
  });
  ...
  it('should return the base prompt when no userMemory is provided', () => {
    ...
    expect(prompt).toContain(
      'You are Perplexity-cli, an interactive CLI agent',
  ...
  describe('PERPLEXITY_SYSTEM_MD environment variable', () => {
    it('should use default prompt when PERPLEXITY_SYSTEM_MD is "false"', () => {
      vi.stubEnv('PERPLEXITY_SYSTEM_MD', 'false');
      const prompt = getCoreSystemPrompt();
      expect(fs.readFileSync).not.toHaveBeenCalled();
      expect(prompt).not.toContain('custom system prompt');
    });

    it('should use default prompt when PERPLEXITY_SYSTEM_MD is "0"', () => {
      vi.stubEnv('PERPLEXITY_SYSTEM_MD', '0');
      const prompt = getCoreSystemPrompt();
      expect(fs.readFileSync).not.toHaveBeenCalled();
      expect(prompt).not.toContain('custom system prompt');
    });

    it('should throw error if PERPLEXITY_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('PERPLEXITY_SYSTEM_MD', customPath);
    ...
    it('should read from default path when PERPLEXITY_SYSTEM_MD is "true"', () => {
      const defaultPath = path.resolve(
        path.join(PERPLEXITY_CONFIG_DIR, 'system.md'),
      );
      vi.stubEnv('PERPLEXITY_SYSTEM_MD', 'true');
    ...
    it('should read from default path when PERPLEXITY_SYSTEM_MD is "1"', () => {
      const defaultPath = path.resolve(
        path.join(PERPLEXITY_CONFIG_DIR, 'system.md'),
      );
      vi.stubEnv('PERPLEXITY_SYSTEM_MD', '1');
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('custom system prompt');
    ...
    it('should read from custom path when PERPLEXITY_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when PERPLEXITY_SYSTEM_MD is set', () => {
  ...
  describe('PERPLEXITY_WRITE_SYSTEM_MD environment variable', () => {
    it('should not write to file when PERPLEXITY_WRITE_SYSTEM_MD is "false"', () => {
      vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', 'false');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).not.toHaveBeenCalled();
    });

    it('should not write to file when PERPLEXITY_WRITE_SYSTEM_MD is "0"', () => {
      vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', '0');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).not.toHaveBeenCalled();
    });

    it('should write to default path when PERPLEXITY_WRITE_SYSTEM_MD is "true"', () => {
      const defaultPath = path.resolve(
        path.join(PERPLEXITY_CONFIG_DIR, 'system.md'),
      );
      vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', 'true');
      getCoreSystemPrompt();
    ...
    it('should write to default path when PERPLEXITY_WRITE_SYSTEM_MD is "1"', () => {
      const defaultPath = path.resolve(
        path.join(PERPLEXITY_CONFIG_DIR, 'system.md'),
      );
      vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', '1');
      getCoreSystemPrompt();
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        defaultPath,
        expect.any(String),
    ...
    it('should write to custom path when PERPLEXITY_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('PERPLEXITY_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt();
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it('should expand tilde in custom path when PERPLEXITY_WRITE_SYSTEM_MD is set', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~/custom/system.md';
      const expectedPath = path.join(homeDir, 'custom/system.md');
    ...
    it('should expand tilde in custom path when PERPLEXITY_WRITE_SYSTEM_MD is just ~', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~';
...
describe('Model-specific tool call formats', () => {
  ...
  it('should use JSON format for perplexity3-coder model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'perplexity3-coder-7b');
  ...
  it('should use JSON format for perplexity-vl model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'perplexity-vl-max');
  ...
  it('should use JSON format for generic models', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
  ...
  it('should use JSON format when no model is specified', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt();

    // Should contain JSON-style tool calls (default behavior)
    expect(prompt).toContain('<tool_call>');
    expect(prompt).toContain('{"name": "');
    expect(prompt).toContain('"arguments"');
  ...
  it('should preserve model-specific formats with user memory', () => {
    ...
    const prompt = getCoreSystemPrompt(userMemory, 'perplexity3-coder-14b');
  ...
  it('should preserve model-specific formats with sandbox environment', () => {
    ...
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'perplexity-vl-plus');
