...
import { Turn, PerplexityEventType } from './turn.js';
import type { GenerateContentResponse, Part, Content } from '@google/genai';
import { reportError } from '../utils/errorReporting.js';
import type { PerplexityChat } from './perplexityChat.js';
import { StreamEventType } from './perplexityChat.js';
...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(
      mockChatInstance as unknown as PerplexityChat,
      'prompt-id-1',
    );
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerPerplexityToolCallRequestEvent;
      expect(event1.type).toBe(PerplexityEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerPerplexityToolCallRequestEvent;
      expect(event2.type).toBe(PerplexityEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: PerplexityEventType.Content, value: 'First part' },
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerPerplexityErrorEvent;
      expect(errorEvent.type).toBe(PerplexityEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerPerplexityToolCallRequestEvent;
    ...
    it('should yield finished event when response has finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: PerplexityEventType.Content, value: 'Partial response' },
        {
    ...
    it('should yield finished event for SAFETY finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: PerplexityEventType.Content, value: 'Content blocked' },
        {
          type: PerplexityEventType.Finished,
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: PerplexityEventType.Content, value: 'First part' },
        { type: PerplexityEventType.Content, value: 'Second part' },
        {
          type: PerplexityEventType.Finished,
    ...
    it('should yield citation and finished events when response has citationMetadata', async () => {
      ...
      expect(events).toEqual([
        { type: PerplexityEventType.Content, value: 'Some text.' },
        {
    ...
    it('should not yield citation event if there is no finish reason', async () => {
      ...
      expect(events.some((e) => e.type === PerplexityEventType.Citation)).toBe(
        false,
      );
    });
    ...
    it('should not crash when cancelled request has malformed error', async () => {
      ...
      expect(events).toEqual([{ type: PerplexityEventType.UserCancelled }]);
    ...
    it('should yield a Retry event when it receives one from the chat stream', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: PerplexityEventType.Content, value: 'Success' },
      ]);
    });
  });
