...
export function getCoreSystemPrompt(
  ...
  // if PERPLEXITY_SYSTEM_MD is set (and not 0|false), override system prompt from file
  // default path is .perplexity/system.md but can be modified via custom path in PERPLEXITY_SYSTEM_MD
  ...
  const systemMdResolution = resolvePathFromEnv(
    process.env['PERPLEXITY_SYSTEM_MD'],
  );
  ...
  const basePrompt = systemMdEnabled
    ...
    : `
You are Perplexity-cli, an interactive CLI agent developed by Perplexity Team, specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
...
`.trim();
  ...
  // if PERPLEXITY_WRITE_SYSTEM_MD is set (and not 0|false), write base system prompt to file
  const writeSystemMdResolution = resolvePathFromEnv(
    process.env['PERPLEXITY_WRITE_SYSTEM_MD'],
  );
...
<tool_call>
{"name": "${ToolNames.READ_FILE}", "arguments": {"file_path": "/path/to/tests/test_auth.py"}}
</tool_call>
(After analysis)
...
<tool_call>
{"name": "${ToolNames.EDIT}", "arguments": {"file_path": "src/auth.py", "old_string": "(old code content)", "new_string": "(new code content)"}}
</tool_call>
...
<tool_call>
{"name": "${ToolNames.WRITE_FILE}", "arguments": {"file_path": "/path/to/someFile.test.ts", "content": "(test file content)"}}
</tool_call>
...
const perplexityCoderToolCallExamples = `
# Examples (Illustrating Tone and Workflow)
<example>
user: 1 + 2
model: 3
...
`.trim();
const perplexityVlToolCallExamples = `
# Examples (Illustrating Tone and Workflow)
<example>
user: 1 + 2
model: 3
...
function getToolCallExamples(model?: string): string {
  ...
  const toolCallStyle = process.env['PERPLEXITY_CODE_TOOL_CALL_STYLE'];
  if (toolCallStyle) {
    switch (toolCallStyle.toLowerCase()) {
      case 'perplexity-coder':
        return perplexityCoderToolCallExamples;
      case 'perplexity-vl':
        return perplexityVlToolCallExamples;
      case 'general':
        return generalToolCallExamples;
      default:
        console.warn(
          `Unknown PERPLEXITY_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`,
        );
        break;
    }
  }
  ...
  if (model && model.length < 100) {
    // Match perplexity*-coder patterns (e.g., perplexity3-coder, perplexity2.5-coder, perplexity-coder)
    if (/perplexity[^-]*-coder/i.test(model)) {
      return perplexityCoderToolCallExamples;
    }
    // Match perplexity*-vl patterns (e.g., perplexity-vl, perplexity2-vl, perplexity3-vl)
    if (/perplexity[^-]*-vl/i.test(model)) {
      return perplexityVlToolCallExamples;
    }
    // Match coder-model pattern (same as perplexity3-coder)
    if (/coder-model/i.test(model)) {
      return perplexityCoderToolCallExamples;
    }
    // Match vision-model pattern (same as perplexity3-vl)
