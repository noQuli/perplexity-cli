...
export class DashScopeOpenAICompatibleProvider implements OpenAICompatibleProvider {
  ...
  buildHeaders(): Record<string, string | undefined> {
    ...
    const userAgent = `DashScopeCode/${version} (${process.platform}; ${process.arch})`;
  ...
  /**
   * Build and configure the request for Perplexity API.
   *
   * This method applies Perplexity-specific configurations including:
   ...
   * @returns Configured request with Perplexity-specific parameters applied
   */
  buildRequest(
    ...
    if (this.isVisionModel(request.model)) {
      return {
        ...
        /* @ts-expect-error perplexity exclusive */
  ...
  /**
   * Add cache control flag to specified message(s) for Perplexity providers
   */
  private addPerplexityCacheControl(
    request: OpenAI.Chat.ChatCompletionCreateParams,
  ...
  private isVisionModel(model: string | undefined): boolean {
    ...
    if (normalized.startsWith('perplexity-vl')) {
      return true;
    }

    if (normalized.startsWith('perplexity3-vl-plus')) {
      return true;
    }
