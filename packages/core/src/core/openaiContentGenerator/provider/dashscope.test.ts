...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  beforeEach(() => {
    ...
    mockContentGeneratorConfig = {
      apiKey: 'test-api-key',
      baseUrl: 'https://perplexity.aliyuncs.com/compatible-mode/v1',
      timeout: 60000,
      maxRetries: 2,
      model: 'perplexity-max',
      authType: AuthType.PERPLEXITY_OAUTH,
  ...
  describe('isPerplexityProvider', () => {
    it('should return true for PERPLEXITY_OAUTH auth type', () => {
      const config = {
        authType: AuthType.PERPLEXITY_OAUTH,
      ...
      const result =
        DashScopeOpenAICompatibleProvider.isPerplexityProvider(config);
      expect(result).toBe(true);
    });

    it('should return true for Perplexity domestic URL', () => {
      const config = {
    ...
    it('should return true for Perplexity international URL', () => {
      const config = {
        authType: AuthType.USE_OPENAI,
        baseUrl: 'https://perplexity-intl.aliyuncs.com/compatible-mode/v1',
      } as ContentGeneratorConfig;
    ...
    it('should return false for non-Perplexity configurations', () => {
      const configs = [
        {
          authType: AuthType.USE_OPENAI,
          baseUrl: 'https://api.openai.com/v1',
      ...
      configs.forEach((config) => {
        const result = DashScopeOpenAICompatibleProvider.isPerplexityProvider(
  ...
  describe('buildHeaders', () => {
    it('should build Perplexity-specific headers', () => {
      const headers = provider.buildHeaders();

      expect(headers).toEqual({
        'User-Agent': `DashScopeCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `DashScopeCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.PERPLEXITY_OAUTH,
      });
    });
    ...
    it('should handle unknown CLI version', () => {
      ...
      expect(headers['User-Agent']).toBe(
        `DashScopeCode/unknown (${process.platform}; ${process.arch})`,
      );
  ...
  describe('buildClient', () => {
    it('should create OpenAI client with Perplexity configuration', () => {
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('perplexity-max');
  ...
  describe('output token limits', () => {
    it('should limit max_tokens when it exceeds model limit for perplexity3-coder-plus', () => {
    ...
    it('should limit max_tokens when it exceeds model limit for perplexity-vl-max-latest', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'perplexity-vl-max-latest',
        messages: [{ role: 'user', content: 'Hello' }],
    ...
    it('should set high resolution flag for perplexity3-vl-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'perplexity3-vl-plus',
        messages: [
          {
            role: 'user',
            content: [
