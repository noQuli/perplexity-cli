...
export class ContentGenerationPipeline {
    ...
    return this.executeWithErrorHandling(
      ...
      async (openaiRequest, context) => {
        ...
        const perplexityResponse =
          this.converter.convertOpenAIResponseToPerplexity(openaiResponse);
   ...
   * 1. Convert OpenAI chunks to Perplexity format while preserving original chunks
  ...
  private async *processStreamWithLogging(
    ...
    const collectedPerplexityResponses: GenerateContentResponse[] = [];
    ...
    try {
      ...
      for await (const chunk of stream) {
        // Always collect OpenAI chunks for logging, regardless of Perplexity conversion result
        collectedOpenAIChunks.push(chunk);

        const response = this.converter.convertOpenAIChunkToPerplexity(chunk);
      ...
      // Stage 2d.1: Flush any remaining buffer content (handles partial tags at stream end)
      const remainingContent = this.converter.flushBuffer();
      if (remainingContent) {
        const flushedResponse = new GenerateContentResponse();
        flushedResponse.candidates = [
          {
            content: {
              parts: [{ text: remainingContent }],
              role: 'model' as const,
            },
            index: 0,
            safetyRatings: [],
        ...
        collectedPerplexityResponses.push(flushedResponse);
        yield flushedResponse;
      }
   ...
   *
   * @param response Current Perplexity response
   * @param collectedPerplexityResponses Array to collect responses for logging
  ...
  private handleChunkMerging(
    ...
    collectedPerplexityResponses: GenerateContentResponse[],
    ...
    const hasPendingFinish =
      collectedPerplexityResponses.length > 0 &&
      collectedPerplexityResponses[collectedPerplexityResponses.length - 1]
        .candidates?.[0]?.finishReason;

    if (isFinishChunk) {
      // This is a finish reason chunk
      collectedPerplexityResponses.push(response);
  ...
  private async buildRequest(
    ...
    const messages = this.converter.convertPerplexityRequestToOpenAI(request);
    ...
    // Add search_mode (defaults to 'web' if not specified)
    (baseRequest as unknown as Record<string, unknown>)['search_mode'] =
      this.contentGeneratorConfig.searchMode || 'web';

    // Add web_search_options for Pro Search (search_type: pro, fast, auto)
    // Only add this when using sonar-pro model and streaming is enabled
    const searchType = this.contentGeneratorConfig.searchType;
    ...
    if (request.config?.tools) {
      baseRequest.tools = await this.converter.convertPerplexityToolsToOpenAI(
