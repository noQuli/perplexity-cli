...
describe('ContentGenerationPipeline', () => {
  ...
  beforeEach(() => {
    ...
    // Mock converter
    mockConverter = {
      convertPerplexityRequestToOpenAI: vi.fn(),
      convertOpenAIResponseToPerplexity: vi.fn(),
      convertOpenAIChunkToPerplexity: vi.fn(),
      convertPerplexityToolsToOpenAI: vi.fn(),
      resetStreamingToolCalls: vi.fn(),
      flushBuffer: vi.fn().mockReturnValue(''),
  ...
  describe('execute', () => {
    it('should successfully execute non-streaming request', async () => {
      ...
      const mockPerplexityResponse = new GenerateContentResponse();

      (mockConverter.convertPerplexityRequestToOpenAI as Mock).mockReturnValue(
        mockMessages,
      );
      (mockConverter.convertOpenAIResponseToPerplexity as Mock).mockReturnValue(
      ...
      // Assert
      expect(result).toBe(mockPerplexityResponse);
      expect(
        mockConverter.convertPerplexityRequestToOpenAI,
      ...
      expect(
        mockConverter.convertOpenAIResponseToPerplexity,
    ...
    it('should handle tools in request', async () => {
      ...
      (mockConverter.convertPerplexityToolsToOpenAI as Mock).mockResolvedValue(
        mockTools,
      );
      (mockConverter.convertOpenAIResponseToPerplexity as Mock).mockReturnValue(
        mockPerplexityResponse,
      ...
      // Assert
      expect(result).toBe(mockPerplexityResponse);
      expect(mockConverter.convertPerplexityToolsToOpenAI).toHaveBeenCalledWith(
  ...
  describe('executeStream', () => {
    it('should successfully execute streaming request', async () => {
      ...
      const mockPerplexityResponse1 = new GenerateContentResponse();
      const mockPerplexityResponse2 = new GenerateContentResponse();
      mockPerplexityResponse1.candidates = [
        { content: { parts: [{ text: 'Hello' }], role: 'model' } },
      ];
      mockPerplexityResponse2.candidates = [
      ...
      (mockConverter.convertOpenAIChunkToPerplexity as Mock)
        .mockReturnValueOnce(mockPerplexityResponse1)
        .mockReturnValueOnce(mockPerplexityResponse2);
      (mockClient.chat.completions.create as Mock).mockResolvedValue(
        mockStream,
      );
      ...
      expect(results[0]).toBe(mockPerplexityResponse1);
      expect(results[1]).toBe(mockPerplexityResponse2);
      ...
      expect(mockTelemetryService.logStreamingSuccess).toHaveBeenCalledWith(
        ...
        [mockPerplexityResponse1, mockPerplexityResponse2],
        expect.any(Object),
    ...
    it('should pass abort signal to OpenAI client for streaming requests', async () => {
      ...
      (mockConverter.convertOpenAIChunkToPerplexity as Mock).mockReturnValue(
        new GenerateContentResponse(),
      );
      (mockClient.chat.completions.create as Mock).mockResolvedValue(
        mockStream,
  ...
  describe('createRequestContext', () => {
    ...
    it('should create context with correct properties for streaming request', async () => {
      ...
      const mockPerplexityResponse = new GenerateContentResponse();
      mockPerplexityResponse.candidates = [
        { content: { parts: [{ text: 'Hello' }], role: 'model' } },
      ];
    ...
    it('should collect all OpenAI chunks for logging even when Perplexity responses are filtered', async () => {
      // Create chunks that would produce empty Perplexity responses (partial tool calls)
      ...
      // Mock empty Perplexity responses for partial chunks (they get filtered)
      const emptyPerplexityResponse1 = new GenerateContentResponse();
      emptyPerplexityResponse1.candidates = [
        {
          content: { parts: [], role: 'model' },
          index: 0,
          safetyRatings: [],
      ...
      const emptyPerplexityResponse2 = new GenerateContentResponse();
      emptyPerplexityResponse2.candidates = [
        {
          content: { parts: [], role: 'model' },
          index: 0,
          safetyRatings: [],
      ...
      // Mock final Perplexity response with tool call
      const finalPerplexityResponse = new GenerateContentResponse();
      finalPerplexityResponse.candidates = [
        {
          content: {
            parts: [
              {
      ...
      (mockConverter.convertOpenAIChunkToPerplexity as Mock)
        .mockReturnValueOnce(emptyPerplexityResponse1) // First partial chunk -> empty response
        .mockReturnValueOnce(emptyPerplexityResponse2) // Second partial chunk -> empty response
        .mockReturnValueOnce(finalPerplexityResponse); // Finish chunk -> complete response
      ...
      expect(responses[0]).toBe(finalPerplexityResponse);
      ...
      expect(mockTelemetryService.logStreamingSuccess).toHaveBeenCalledWith(
        ...
        [finalPerplexityResponse], // Only the non-empty Perplexity response
        expect.objectContaining({
          model: 'test-model',
