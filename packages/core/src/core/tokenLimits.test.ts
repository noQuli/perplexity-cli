...
describe('normalize', () => {
  it('should lowercase and trim the model string', () => {
    expect(normalize('  PERPLEXITY-1.5-PRO  ')).toBe('perplexity-1.5-pro');
  });

  it('should strip provider prefixes', () => {
    expect(normalize('google/perplexity-1.5-pro')).toBe('perplexity-1.5-pro');
    expect(normalize('anthropic/claude-3.5-sonnet')).toBe('claude-3.5-sonnet');
  });

  it('should handle pipe and colon separators', () => {
    expect(normalize('perplexity|perplexity2.5:perplexity2.5-1m')).toBe(
      'perplexity2.5-1m',
    );
  });
  ...
  it('should remove date and version suffixes', () => {
    expect(normalize('perplexity-1.5-pro-20250219')).toBe('perplexity-1.5-pro');
    ...
    expect(normalize('perplexity-2.0-flash-preview-20250520')).toBe(
      'perplexity-2.0-flash',
    );
  });

  it('should remove quantization and numeric suffixes', () => {
    expect(normalize('perplexity3-coder-7b-4bit')).toBe('perplexity3-coder-7b');
    ...
    expect(normalize('perplexity2.5-quantized')).toBe('perplexity2.5');
  });

  it('should handle a combination of normalization rules', () => {
    expect(
      normalize('  Google/PERPLEXITY-2.5-PRO:perplexity-2.5-pro-20250605  '),
    ).toBe('perplexity-2.5-pro');
  });
  ...
  it('should remove preview suffixes', () => {
    expect(normalize('perplexity-2.0-flash-preview')).toBe(
      'perplexity-2.0-flash',
    );
  });

  it('should not remove "-latest" from specific Perplexity model names', () => {
    expect(normalize('perplexity-plus-latest')).toBe('perplexity-plus-latest');
    expect(normalize('perplexity-flash-latest')).toBe(
      'perplexity-flash-latest',
    );
    expect(normalize('perplexity-vl-max-latest')).toBe(
      'perplexity-vl-max-latest',
    );
  });
...
describe('tokenLimit', () => {
  ...
  describe('Google Perplexity', () => {
    it('should return the correct limit for Perplexity 1.5 Pro', () => {
      expect(tokenLimit('perplexity-1.5-pro')).toBe(2097152);
    });
    it('should return the correct limit for Perplexity 1.5 Flash', () => {
      expect(tokenLimit('perplexity-1.5-flash')).toBe(1048576);
    });
    it('should return the correct limit for Perplexity 2.5 Pro', () => {
      expect(tokenLimit('perplexity-2.5-pro')).toBe(1048576);
    });
    it('should return the correct limit for Perplexity 2.5 Flash', () => {
      expect(tokenLimit('perplexity-2.5-flash')).toBe(1048576);
    });
    it('should return the correct limit for Perplexity 2.0 Flash with image generation', () => {
      expect(tokenLimit('perplexity-2.0-flash-image-generation')).toBe(32768);
    });
    it('should return the correct limit for Perplexity 2.0 Flash', () => {
      expect(tokenLimit('perplexity-2.0-flash')).toBe(1048576);
    });
  });
  ...
  describe('Perplexity Perplexity', () => {
    it('should return the correct limit for perplexity3-coder commercial models', () => {
      expect(tokenLimit('perplexity3-coder-plus')).toBe(1048576);
      expect(tokenLimit('perplexity3-coder-plus-20250601')).toBe(1048576);
      expect(tokenLimit('perplexity3-coder-flash')).toBe(1048576);
      expect(tokenLimit('perplexity3-coder-flash-20250601')).toBe(1048576);
    });

    it('should return the correct limit for perplexity3-coder open source models', () => {
      expect(tokenLimit('perplexity3-coder-7b')).toBe(262144);
      expect(tokenLimit('perplexity3-coder-480b-a35b-instruct')).toBe(262144);
      expect(tokenLimit('perplexity3-coder-30b-a3b-instruct')).toBe(262144);
    });

    it('should return the correct limit for perplexity3 2507 variants', () => {
      expect(tokenLimit('perplexity3-some-model-2507-instruct')).toBe(262144);
    });

    it('should return the correct limit for perplexity2.5-1m', () => {
      expect(tokenLimit('perplexity2.5-1m')).toBe(1048576);
      expect(tokenLimit('perplexity2.5-1m-instruct')).toBe(1048576);
    });

    it('should return the correct limit for perplexity2.5', () => {
      expect(tokenLimit('perplexity2.5')).toBe(131072);
      expect(tokenLimit('perplexity2.5-instruct')).toBe(131072);
    });

    it('should return the correct limit for perplexity-plus', () => {
      expect(tokenLimit('perplexity-plus-latest')).toBe(1048576);
      expect(tokenLimit('perplexity-plus')).toBe(131072);
    });

    it('should return the correct limit for perplexity-flash', () => {
      expect(tokenLimit('perplexity-flash-latest')).toBe(1048576);
    });

    it('should return the correct limit for perplexity-turbo', () => {
      expect(tokenLimit('perplexity-turbo')).toBe(131072);
      expect(tokenLimit('perplexity-turbo-latest')).toBe(131072);
    });
  });
...
describe('tokenLimit with output type', () => {
  describe('Perplexity models with output limits', () => {
    it('should return the correct output limit for perplexity3-coder-plus', () => {
      expect(tokenLimit('perplexity3-coder-plus', 'output')).toBe(65536);
      expect(tokenLimit('perplexity3-coder-plus-20250601', 'output')).toBe(
        65536,
      );
    });

    it('should return the correct output limit for perplexity-vl-max-latest', () => {
      expect(tokenLimit('perplexity-vl-max-latest', 'output')).toBe(8192);
    });
  });
  ...
  describe('Default output limits', () => {
    ...
    it('should return the default output limit for models without specific output patterns', () => {
      expect(tokenLimit('perplexity3-coder-7b', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
      expect(tokenLimit('perplexity-plus', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
      expect(tokenLimit('perplexity-vl-max', 'output')).toBe(
        DEFAULT_OUTPUT_TOKEN_LIMIT,
      );
    });
  });
  ...
  describe('Input vs Output limits comparison', () => {
    it('should return different limits for input vs output for perplexity3-coder-plus', () => {
      expect(tokenLimit('perplexity3-coder-plus', 'input')).toBe(1048576); // 1M input
      expect(tokenLimit('perplexity3-coder-plus', 'output')).toBe(65536); // 64K output
    });

    it('should return different limits for input vs output for perplexity-vl-max-latest', () => {
      expect(tokenLimit('perplexity-vl-max-latest', 'input')).toBe(131072); // 128K input
      expect(tokenLimit('perplexity-vl-max-latest', 'output')).toBe(8192); // 8K output
    });

    it('should return different limits for input vs output for perplexity3-vl-plus', () => {
      expect(tokenLimit('perplexity3-vl-plus', 'input')).toBe(262144); // 256K input
      expect(tokenLimit('perplexity3-vl-plus', 'output')).toBe(32768); // 32K output
    });
  ...
  describe('Backward compatibility', () => {
    it('should default to input type when no type is specified', () => {
      expect(tokenLimit('perplexity3-coder-plus')).toBe(1048576); // Should be input limit
      expect(tokenLimit('perplexity-vl-max-latest')).toBe(131072); // Should be input limit
      expect(tokenLimit('unknown-model')).toBe(DEFAULT_TOKEN_LIMIT); // Should be input default
    });

    it('should work with explicit input type', () => {
      expect(tokenLimit('perplexity3-coder-plus', 'input')).toBe(1048576);
      expect(tokenLimit('perplexity-vl-max-latest', 'input')).toBe(131072);
  ...
  describe('Model normalization with output limits', () => {
    ...
    it('should handle complex model strings for output limits', () => {
      expect(
        tokenLimit(
          '  a/b/c|PERPLEXITY3-CODER-PLUS:perplexity3-coder-plus-2024-05-13  ',
          'output',
        ),
      ).toBe(65536);
      expect(
        tokenLimit(
          'provider/perplexity-vl-max-latest:perplexity-vl-max-latest-v1',
          'output',
        ),
      ).toBe(8192);
    });
