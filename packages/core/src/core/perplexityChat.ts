 ...
 *
 * The Perplexity provider may return the last 2 chunks as:
...
export class PerplexityChat {
  ...
  /**
   * Creates a new PerplexityChat instance.
   *
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'perplexity-2.0-flash'});
  ...
  private async makeApiCallAndProcessStream(
    ...
    const apiCall = () => {
      ...
      if (
        ...
        modelToUse === DEFAULT_PERPLEXITY_FLASH_MODEL
      ) {
        throw new Error(
