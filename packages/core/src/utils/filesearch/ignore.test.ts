...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .perplexityignore', async () => {
    tmpDir = await createTmpDir({
      '.perplexityignore': '*.log',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: false,
  ...
  it('should combine rules from .gitignore and .perplexityignore', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': '*.log',
      '.perplexityignore': '*.txt',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: true,
