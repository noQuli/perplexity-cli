...
import { DEFAULT_PERPLEXITY_FLASH_MODEL } from '../config/models.js';
...
describe('parseAndFormatApiError', () => {
  ...
  const perplexityMessage = 'request a quota increase through AI Studio';
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the sonar model',
    );
  });
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "Perplexity 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/perplexity-api/docs/rate-limits.",
    ...
    expect(result).toContain('Perplexity 2.5 Pro Preview');
    expect(result).toContain(perplexityMessage);
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Free tier)', () => {
    const errorMessage =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Perplexity 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const result = parseAndFormatApiError(
      errorMessage,
      AuthType.LOGIN_WITH_GOOGLE,
      undefined,
    ...
    expect(result).toContain(
      "[API Error: Quota exceeded for quota metric 'Perplexity 2.5 Pro Requests'",
    );
    expect(result).toContain(
      'You have reached your daily sonar-pro quota limit',
    );
  ...
  it('should format a 429 API error with generic quota exceeded message for Google auth', () => {
    ...
    expect(result).not.toContain(
      'You have reached your daily Perplexity 2.5 Pro quota limit',
    );
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Standard tier)', () => {
    ...
    expect(result).toContain(
      'We appreciate you for choosing Perplexity Code Assist and the Perplexity CLI',
    );
  ...
  it('should handle different Perplexity 2.5 version strings in Pro quota exceeded errors', () => {
    const errorMessage25 =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Perplexity 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const errorMessagePreview =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Perplexity 2.5-preview Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    ...
    expect(resultPreview).toContain(
      'You have reached your daily perplexity-2.5-preview-pro quota limit',
    );
  ...
  it('should not match non-Pro models with similar version strings', () => {
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Perplexity 2.5 Flash Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Perplexity 2.5-preview Flash Requests' and limit",
      ),
    ).toBe(false);
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Perplexity 2.5 Ultra Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Perplexity 2.5 Standard Requests' and limit",
      ),
    ).toBe(false);
