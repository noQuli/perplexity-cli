...
import { getAllPerplexityMdFilenames } from '../tools/memoryTool.js';
...
interface PerplexityFileContent {
  filePath: string;
  content: string | null;
}
...
async function getPerplexityMdFilePathsInternal(
  ...
  for (let i = 0; i < dirsArray.length; i += CONCURRENT_LIMIT) {
    ...
    const batchPromises = batch.map((dir) =>
      getPerplexityMdFilePathsInternalForEachDir(
        dir,
        userHomePath,
        debugMode,
        fileService,
...
async function getPerplexityMdFilePathsInternalForEachDir(
  dir: string,
  userHomePath: string,
  debugMode: boolean,
  fileService: FileDiscoveryService,
  ...
  const perplexityMdFilenames = getAllPerplexityMdFilenames();

  for (const perplexityMdFilename of perplexityMdFilenames) {
    const resolvedHome = path.resolve(userHomePath);
    const globalMemoryPath = path.join(
      resolvedHome,
      PERPLEXITY_DIR,
      perplexityMdFilename,
    );
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${perplexityMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for PERPLEXITY.md directly in the home directory
      const homeContextPath = path.join(resolvedHome, perplexityMdFilename);
      try {
        ...
        if (homeContextPath !== globalMemoryPath) {
          ...
          if (debugMode)
            logger.debug(
              `Found readable home ${perplexityMdFilename}: ${homeContextPath}`,
            );
        }
      } catch {
      ...
      if (debugMode)
        logger.debug(
          `Searching for ${perplexityMdFilename} starting from CWD: ${resolvedCwd}`,
        );
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, PERPLEXITY_DIR)) {
          break;
        }

        const potentialPath = path.join(currentDir, perplexityMdFilename);
        try {
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllPerplexityMdFilenames()} paths to read: ${JSON.stringify(
        finalPaths,
      )}`,
    );
  return finalPaths;
...
async function readPerplexityMdFiles(
  filePaths: string[],
  debugMode: boolean,
  importFormat: 'flat' | 'tree' = 'tree',
): Promise<PerplexityFileContent[]> {
  ...
  const results: PerplexityFileContent[] = [];

  for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
    const batch = filePaths.slice(i, i + CONCURRENT_LIMIT);
    const batchPromises = batch.map(
      async (filePath): Promise<PerplexityFileContent> => {
        try {
          ...
          if (!isTestEnv) {
            ...
            logger.warn(
              `Warning: Could not read ${getAllPerplexityMdFilenames()} file at ${filePath}. Error: ${message}`,
            );
          }
...
function concatenateInstructions(
  instructionContents: PerplexityFileContent[],
...
/**
 * Loads hierarchical PERPLEXITY.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  const filePaths = await getPerplexityMdFilePathsInternal(
    currentWorkingDirectory,
    includeDirectoriesToReadPerplexity,
    userHomePath,
    debugMode,
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No PERPLEXITY.md files found in hierarchy.');
    return { memoryContent: '', fileCount: 0 };
  }
  const contentsWithPaths = await readPerplexityMdFiles(
    filePaths,
    debugMode,
    importFormat,
  );
