...
// Mock PerplexityClient and Config constructor
...
describe('summarizers', () => {
  ...
  beforeEach(() => {
    ...
    mockPerplexityClient = new PerplexityClient(mockConfigInstance);
    (mockPerplexityClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockPerplexityClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockPerplexityClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockPerplexityClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockPerplexityClient.generateContent as Mock).mockRejectedValue(error);
...
`;
      const calledWith = (mockPerplexityClient.generateContent as Mock).mock
        .calls[0];
