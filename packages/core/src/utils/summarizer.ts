...
import type { PerplexityClient } from '../core/client.js';
import { DEFAULT_PERPLEXITY_FLASH_LITE_MODEL } from '../config/models.js';
 ...
 *
 * @param result The result of the tool execution.
 * @param perplexityClient The Perplexity client to use for summarization.
...
export async function summarizeToolOutput(
  ...
  try {
    const parsedResponse = (await perplexityClient.generateContent(
      contents,
      toolOutputSummarizerConfig,
      abortSignal,
      DEFAULT_PERPLEXITY_FLASH_LITE_MODEL,
