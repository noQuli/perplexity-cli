...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile('.perplexity', 'config.yaml');
      await createTestFile('.perplexity', 'logs.json');
  ...
  describe('with perplexityignore', () => {
    it('should ignore perplexityignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.perplexityignore'),
        'ignored.txt\nnode_modules/\n.perplexity/\n!/.perplexity/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectPerplexityIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.perplexityignore'),
        'ignored.txt\nnode_modules/\n.perplexity/\n!/.perplexity/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectPerplexityIgnore: false,
          respectGitIgnore: true, // Explicitly disable perplexity ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
