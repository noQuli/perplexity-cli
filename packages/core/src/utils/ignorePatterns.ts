...
import { getCurrentPerplexityMdFilename } from '../tools/memoryTool.js';
...
export interface ExcludeOptions {
  ...
  /**
   * Whether to include dynamic patterns like the current Perplexity MD filename. Defaults to true.
   */
...
export class FileExclusions {
  ...
  getDefaultExcludePatterns(options: ExcludeOptions = {}): string[] {
    ...
    // Add dynamic patterns (like current Perplexity MD filename)
    if (includeDynamicPatterns) {
      patterns.push(`**/${getCurrentPerplexityMdFilename()}`);
    }
