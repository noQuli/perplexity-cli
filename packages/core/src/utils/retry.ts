...
export async function retryWithBackoff<T>(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      // Check for Perplexity OAuth quota exceeded error - throw immediately without retry
      if (
        authType === AuthType.PERPLEXITY_OAUTH &&
        isPerplexityQuotaExceededError(error)
      ) {
        throw new Error(
          `Perplexity API quota exceeded: Your Perplexity API quota has been exhausted. Please wait for your quota to reset.`,
        );
      }

      // Track consecutive 429 errors, but handle Perplexity throttling differently
      if (errorStatus === 429) {
        // For Perplexity throttling errors, we still want to track them for exponential backoff
        // but not for quota fallback logic (since Perplexity doesn't have model fallback)
        if (
          authType === AuthType.PERPLEXITY_OAUTH &&
          isPerplexityThrottlingError(error)
        ) {
