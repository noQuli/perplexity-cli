...
import {
  setPerplexityMdFilename,
  DEFAULT_CONTEXT_FILENAME,
} from '../tools/memoryTool.js';
import { FileDiscoveryService } from '../services/fileDiscoveryService.js';
import { PERPLEXITY_DIR } from './paths.js';
...
describe('loadServerHierarchicalMemory', () => {
  ...
  afterEach(async () => {
    ...
    setPerplexityMdFilename(DEFAULT_CONTEXT_FILENAME);
  ...
  it('should load only the global context file if present and others are not (default filename)', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, PERPLEXITY_DIR, DEFAULT_CONTEXT_FILENAME),
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setPerplexityMdFilename(customFilename);

    const customContextFile = await createTestFile(
      path.join(homedir, PERPLEXITY_DIR, customFilename),
  ...
  it('should load ORIGINAL_PERPLEXITY_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootPerplexityFile = await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      'Project root memory',
    );
    const srcPerplexityFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'Src directory memory',
    );
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, projectRootPerplexityFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(cwd, projectRootPerplexityFile)} ---\n\n--- Context from: ${path.relative(cwd, srcPerplexityFile)} ---\nSrc directory memory\n--- End of Context from: ${path.relative(cwd, srcPerplexityFile)} ---`,
      fileCount: 2,
    });
  });

  it('should load ORIGINAL_PERPLEXITY_MD_FILENAME files by downward traversal from CWD', async () => {
    await createTestFile(
  ...
  it('should load and correctly order global, upward, and downward ORIGINAL_PERPLEXITY_MD_FILENAME files', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, PERPLEXITY_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
    const rootPerplexityFile = await createTestFile(
    ...
    const cwdPerplexityFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'CWD memory',
    );
    const subDirPerplexityFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, defaultContextFile)} ---\ndefault context content\n--- End of Context from: ${path.relative(cwd, defaultContextFile)} ---\n\n--- Context from: ${path.relative(cwd, rootPerplexityFile)} ---\nProject parent memory\n--- End of Context from: ${path.relative(cwd, rootPerplexityFile)} ---\n\n--- Context from: ${path.relative(cwd, projectRootPerplexityFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(cwd, projectRootPerplexityFile)} ---\n\n--- Context from: ${path.relative(cwd, cwdPerplexityFile)} ---\nCWD memory\n--- End of Context from: ${path.relative(cwd, cwdPerplexityFile)} ---\n\n--- Context from: ${path.relative(cwd, subDirPerplexityFile)} ---\nSubdir memory\n--- End of Context from: ${path.relative(cwd, subDirPerplexityFile)} ---`,
      fileCount: 5,
    });
  });
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const regularSubDirPerplexityFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, regularSubDirPerplexityFile)} ---\nMy code memory\n--- End of Context from: ${path.relative(cwd, regularSubDirPerplexityFile)} ---`,
      fileCount: 1,
    });
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = await createTestFile(
      path.join(testRootDir, 'extensions/ext1/PERPLEXITY.md'),
  ...
  it('should handle multiple directories and files in parallel correctly', async () => {
    // Create multiple test directories with PERPLEXITY.md files
    const numDirs = 5;
