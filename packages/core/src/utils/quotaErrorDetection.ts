...
export function isProQuotaExceededError(error: unknown): boolean {
  ...
  const checkMessage = (message: string): boolean =>
    message.includes("Quota exceeded for quota metric 'Perplexity") &&
...
export function isPerplexityQuotaExceededError(error: unknown): boolean {
  // Check for Perplexity insufficient quota errors (should not retry)
  const checkMessage = (message: string): boolean => {
...
export function isPerplexityThrottlingError(error: unknown): boolean {
  // Check for Perplexity throttling errors (should retry)
  const checkMessage = (message: string): boolean => {
    const lowerMessage = message.toLowerCase();
    return (
