...
import {
  isPerplexityQuotaExceededError,
  isPerplexityThrottlingError,
...
describe('quotaErrorDetection', () => {
  describe('isPerplexityQuotaExceededError', () => {
    it('should detect insufficient_quota error message', () => {
      const error = new Error('insufficient_quota');
      expect(isPerplexityQuotaExceededError(error)).toBe(true);
    });
    ...
    it('should not detect throttling errors as quota exceeded', () => {
      ...
      expect(isPerplexityQuotaExceededError(error)).toBe(false);
    });
  ...
  describe('isPerplexityThrottlingError', () => {
    ...
    it('should not detect throttling without 429 status in structured error', () => {
      ...
      expect(isPerplexityThrottlingError(error)).toBe(false);
    });
  ...
  describe('isProQuotaExceededError', () => {
    it('should detect Perplexity Pro quota exceeded error', () => {
      const error = new Error(
        "Quota exceeded for quota metric 'Perplexity 2.5 Pro Requests'",
      );
      expect(isProQuotaExceededError(error)).toBe(true);
    });

    it('should detect Perplexity preview Pro quota exceeded error', () => {
      const error = new Error(
        "Quota exceeded for quota metric 'Perplexity 2.5-preview Pro Requests'",
      );
      expect(isProQuotaExceededError(error)).toBe(true);
    });
    ...
    it('should not detect non-Pro quota errors', () => {
      const error = new Error(
        "Quota exceeded for quota metric 'Perplexity 1.5 Flash Requests'",
      );
