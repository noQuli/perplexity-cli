...
export function query({
  ...
  /**
   * The prompt to send to the Perplexity Code CLI process.
  ...
  const pathToPerplexityExecutable =
    options.pathToPerplexityExecutable ?? parsedExecutable.executablePath;

  const abortController = options.abortController ?? new AbortController();

  const transport = new ProcessTransport({
    pathToPerplexityExecutable,
    cwd: options.cwd,
    model: options.model,
...
function validateOptions(
  ...
  try {
    parsedExecutable = parseExecutableSpec(options.pathToPerplexityExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToPerplexityExecutable: ${errorMessage}`);
  }
