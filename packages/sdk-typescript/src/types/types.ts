...
export type TransportOptions = {
  pathToPerplexityExecutable: string;
  cwd?: string;
  model?: string;
 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @perplexity-cli/perplexity-cli-core.
 */
...
/**
 * Configuration options for creating a query session with the Perplexity CLI.
 */
export interface QueryOptions {
   ...
   * This takes precedence over the environment variables `OPENAI_MODEL` and `PERPLEXITY_MODEL`
   * @example 'perplexity-max', 'perplexity-plus', 'perplexity-turbo'
   */
  model?: string;

  /**
   * Path to the Perplexity CLI executable or runtime specification.
   *
   ...
   * 1. PERPLEXITY_CODE_CLI_PATH environment variable
   * 2. ~/.volta/bin/perplexity
   * 3. ~/.npm-global/bin/perplexity
   * 4. /usr/local/bin/perplexity
   * 5. ~/.local/bin/perplexity
   * 6. ~/node_modules/.bin/perplexity
   * 7. ~/.yarn/bin/perplexity
   *
   * The .ts files are only supported for debugging purposes.
   *
   * @example 'perplexity'
   * @example '/usr/local/bin/perplexity'
   * @example 'tsx:/path/to/packages/cli/src/index.ts'
   */
  pathToPerplexityExecutable?: string;

  /**
   * Environment variables to pass to the Perplexity CLI process.
  ...
  /**
   * Custom handler for stderr output from the Perplexity CLI process.
   ...
   * - 'perplexity-oauth': Use Perplexity OAuth authentication
   *
   * Though we support 'perplexity-oauth', it's not recommended to use it in the SDK.
   * Because the credentials are stored in `~/.perplexity` and may need to refresh periodically.
   */
  authType?: 'openai' | 'perplexity-oauth';
