...
A minimum experimental TypeScript SDK for programmatic access to Perplexity CLI.
...
```bash
npm install @perplexity-cli/sdk
```

## Requirements

- Node.js >= 20.0.0
- [Perplexity CLI](https://github.com/NoQuli/perplexity-cli) >= 0.4.0 (stable) installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the Perplexity CLI executable. You should explicitly set the `pathToPerplexityExecutable` option to the full path of the `perplexity` binary.

## Quick Start

```typescript
import { query } from '@perplexity-cli/sdk';
...
Creates a new query session with the Perplexity CLI.
...
| `model`                      | `string`                                       | -                | The AI model to use (e.g., `'perplexity-max'`, `'perplexity-plus'`, `'perplexity-turbo'`). Takes precedence over `OPENAI_MODEL` and `PERPLEXITY_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                                                                         |
| `pathToPerplexityExecutable` | `string`                                       | Auto-detected    | Path to the Perplexity CLI executable. Supports multiple formats: `'perplexity'` (native binary from PATH), `'/path/to/perplexity'` (explicit path), `'/path/to/perplexity.js'` (Node.js bundle), `'node:/path/to/perplexity.js'` (force Node.js runtime), `'bun:/path/to/perplexity.js'` (force Bun runtime). If not provided, auto-detects from: `PERPLEXITY_CODE_CLI_PATH` env var, `~/.volta/bin/perplexity`, `~/.npm-global/bin/perplexity`, `/usr/local/bin/perplexity`, `~/.local/bin/perplexity`, `~/node_modules/.bin/perplexity`, `~/.yarn/bin/perplexity`. |
...
| `env`                        | `Record<string, string>`                       | -                | Environment variables to pass to the Perplexity CLI process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `authType`                   | `'openai' \| 'perplexity-oauth'`               | `'openai'`       | Authentication type for the AI service. Using `'perplexity-oauth'` in SDK is not recommended as credentials are stored in `~/.perplexity` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                                                                                              |
...
```typescript
const query = perplexity.query('Your prompt', {
  timeout: {
...
await q.setModel('perplexity-max');
...
```typescript
import { query, type SDKUserMessage } from '@perplexity-cli/sdk';
...
```typescript
import { query, type CanUseTool } from '@perplexity-cli/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@perplexity-cli/sdk';
...
```typescript
import { query, isAbortError } from '@perplexity-cli/sdk';
...
```typescript
import { AbortError, isAbortError } from '@perplexity-cli/sdk';
