...
describe('CLI Path Utilities', () => {
  ...
  describe('parseExecutableSpec', () => {
    describe('auto-detection (no spec provided)', () => {
      it('should auto-detect native CLI when no spec provided', () => {
        ...
        const originalEnv = process.env['PERPLEXITY_CODE_CLI_PATH'];
        process.env['PERPLEXITY_CODE_CLI_PATH'] = '/usr/local/bin/perplexity';
        mockFs.existsSync.mockReturnValue(true);

        const result = parseExecutableSpec();

        expect(result).toEqual({
          executablePath: path.resolve('/usr/local/bin/perplexity'),
          isExplicitRuntime: false,
        });

        // Restore env
        process.env['PERPLEXITY_CODE_CLI_PATH'] = originalEnv;
      });
      ...
      it('should throw when auto-detection fails', () => {
        ...
        expect(() => parseExecutableSpec()).toThrow(
          'perplexity CLI not found. Please:',
        );
      });
    });
    ...
    describe('command name detection', () => {
      it('should detect command names without path separators', () => {
        const result = parseExecutableSpec('perplexity');

        expect(result).toEqual({
          executablePath: 'perplexity',
          isExplicitRuntime: false,
        });
      });

      it('should detect command names on Windows', () => {
        const result = parseExecutableSpec('perplexity.exe');

        expect(result).toEqual({
          executablePath: 'perplexity.exe',
          isExplicitRuntime: false,
        });
      });
    });
    ...
    describe('file path resolution', () => {
      it('should resolve absolute file paths', () => {
        ...
        const result = parseExecutableSpec('/absolute/path/to/perplexity');

        expect(result).toEqual({
          executablePath: path.resolve('/absolute/path/to/perplexity'),
          isExplicitRuntime: false,
        });
      });
      ...
      it('should resolve relative file paths', () => {
        ...
        const result = parseExecutableSpec('./relative/path/to/perplexity');

        expect(result).toEqual({
          executablePath: path.resolve('./relative/path/to/perplexity'),
          isExplicitRuntime: false,
        });
      });
  ...
  describe('prepareSpawnInfo', () => {
    ...
    describe('native executables', () => {
      it('should prepare spawn info for native binary command', () => {
        const result = prepareSpawnInfo('perplexity');

        expect(result).toEqual({
          command: 'perplexity',
          args: [],
          type: 'native',
          originalInput: 'perplexity',
        });
      });

      it('should prepare spawn info for native binary path', () => {
        const result = prepareSpawnInfo('/usr/local/bin/perplexity');

        expect(result).toEqual({
          command: path.resolve('/usr/local/bin/perplexity'),
          args: [],
          type: 'native',
          originalInput: '/usr/local/bin/perplexity',
        });
      });
    });
  ...
  describe('findNativeCliPath', () => {
    it('should find CLI from environment variable', () => {
      const originalEnv = process.env['PERPLEXITY_CODE_CLI_PATH'];
      process.env['PERPLEXITY_CODE_CLI_PATH'] = '/custom/path/to/perplexity';
      mockFs.existsSync.mockReturnValue(true);

      const result = findNativeCliPath();

      expect(result).toBe(path.resolve('/custom/path/to/perplexity'));
  ...
  describe('real-world use cases', () => {
    ...
    it('should handle development with TypeScript source', () => {
      const devPath = '/Users/dev/perplexity-code/packages/cli/index.ts';
  ...
  describe('comprehensive validation', () => {
    ...
    describe('command name validation', () => {
      ...
      it('should reject invalid command name characters', () => {
        expect(() => parseExecutableSpec('perplexity@invalid')).toThrow(
          "Invalid command name 'perplexity@invalid'. Command names should only contain letters, numbers, dots, hyphens, and underscores.",
        );

        expect(() => parseExecutableSpec('perplexity/invalid')).not.toThrow(); // This is treated as a path
      });

      it('should accept valid command names', () => {
        expect(() => parseExecutableSpec('perplexity')).not.toThrow();
        expect(() => parseExecutableSpec('perplexity-code')).not.toThrow();
        expect(() => parseExecutableSpec('perplexity_code')).not.toThrow();
        expect(() => parseExecutableSpec('perplexity.exe')).not.toThrow();
        expect(() => parseExecutableSpec('perplexity123')).not.toThrow();
      });
    });
    ...
    describe('file path validation', () => {
      ...
      it('should accept valid file paths', () => {
        ...
        expect(() => parseExecutableSpec('/path/to/perplexity')).not.toThrow();
