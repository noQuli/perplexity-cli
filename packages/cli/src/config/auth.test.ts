...
describe('validateAuthMethod', () => {
  ...
  it('should return null for PERPLEXITY_API_KEY', () => {
    process.env['PERPLEXITY_API_KEY'] = 'fake-key';
    expect(validateAuthMethod(AuthType.PERPLEXITY_API_KEY)).toBeNull();
  });

  it('should return an error message for an invalid auth method', () => {
    expect(validateAuthMethod('invalid-method')).toBe(
      'Invalid auth method selected. Perplexity CLI only supports API key authentication.',
    );
  });
});
