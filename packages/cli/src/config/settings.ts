...
export const SETTINGS_DIRECTORY_NAME = '.perplexity';
...
export function getSystemSettingsPath(): string {
  if (process.env['PERPLEXITY_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['PERPLEXITY_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/PerplexityCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\perplexity-code\\settings.json';
  } else {
    return '/etc/perplexity-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['PERPLEXITY_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['PERPLEXITY_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer perplexity-specific .env under PERPLEXITY_DIR
    const perplexityEnvPath = path.join(currentDir, PERPLEXITY_DIR, '.env');
    if (fs.existsSync(perplexityEnvPath)) {
      return perplexityEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring perplexity-specific .env
      const homePerplexityEnvPath = path.join(
        homedir(),
        PERPLEXITY_DIR,
        '.env',
      );
      if (fs.existsSync(homePerplexityEnvPath)) {
        return homePerplexityEnvPath;
      }
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(PERPLEXITY_DIR);
