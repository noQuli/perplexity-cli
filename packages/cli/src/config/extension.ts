...
export const EXTENSIONS_CONFIG_FILENAME = 'perplexity-extension.json';
export const INSTALL_METADATA_FILENAME = '.perplexity-extension-install.json';
...
export class ExtensionStorage {
  ...
  static async createTmpDir(): Promise<string> {
    return await fs.promises.mkdtemp(
      path.join(os.tmpdir(), 'perplexity-extension'),
    );
  }
}
...
function extensionConsentString(extensionConfig: ExtensionConfig): string {
  ...
  if (extensionConfig.contextFileName) {
    output.push(
      `This extension will append info to your perplexity.md context using ${extensionConfig.contextFileName}`,
    );
  }
