...
const EXTENSIONS_DIRECTORY_NAME = path.join(PERPLEXITY_DIR, 'extensions');
...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'perplexity-code-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'perplexity-code-test-workspace-'),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when PERPLEXITY.md is present', () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'PERPLEXITY.md'),
      ]);
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error and cleanup if perplexity-extension.json is missing', async () => {
    ...
    it('should throw an error for invalid JSON in perplexity-extension.json', async () => {
    ...
    it('should throw an error for missing name in perplexity-extension.json', async () => {
      const sourceExtDir = createExtension({
        extensionsDir: tempHomeDir,
    ...
    it('should install an extension from a git URL', async () => {
      const gitUrl = 'https://github.com/google/perplexity-extensions.git';
      const extensionName = 'perplexity-extensions';
  ...
  * test-server-2 (remote): https://google.com`,
  ...
  describe('uninstallExtension', () => {
    ...
    it('should uninstall an extension by its source URL', async () => {
      const gitUrl = 'https://github.com/google/perplexity-sql-extension.git';
      const sourceExtDir = createExtension({
        extensionsDir: userExtensionsDir,
        name: 'perplexity-sql-extension',
        version: '1.0.0',
        installMetadata: {
          source: gitUrl,
          type: 'git',
  ...
  describe('enableExtension', () => {
    ...
    const getActiveExtensions = (): PerplexityCLIExtension[] => {
      const manager = new ExtensionEnablementManager(
        ExtensionStorage.getUserExtensionsDir(),
      );
      const extensions = loadExtensions(manager);
