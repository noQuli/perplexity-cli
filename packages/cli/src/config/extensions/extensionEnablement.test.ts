...
function createTestDir() {
  const dirPath = fs.mkdtempSync(path.join(os.tmpdir(), 'perplexity-test-'));
  return {
    path: dirPath,
...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    configDir = path.join(testDir.path, '.perplexity');
  ...
  it('should enable a path based on an enable override', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/perplexity-cli');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/perplexity-cli')).toBe(
      true,
    );
  });
