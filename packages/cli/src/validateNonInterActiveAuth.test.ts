...
describe('validateNonInterActiveAuth', () => {
  let originalEnvPerplexityApiKey: string | undefined;
  ...
  afterEach(() => {
    if (originalEnvPerplexityApiKey !== undefined) {
      process.env['PERPLEXITY_API_KEY'] = originalEnvPerplexityApiKey;
    } else {
      delete process.env['PERPLEXITY_API_KEY'];
    }
  ...
  it('uses configured USE_PERPLEXITY if provided', async () => {
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.USE_PERPLEXITY);
  });
