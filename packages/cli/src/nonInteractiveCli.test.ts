...
vi.mock('@perplexity-cli/perplexity-cli-core', async (importOriginal) => {
  const original =
    await importOriginal<
      typeof import('@perplexity-cli/perplexity-cli-core')
    >();
...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getPerplexityClient: vi.fn().mockReturnValue(mockPerplexityClient),
      ...
      storage: {
        getProjectTempDir: vi
          .fn()
          .mockReturnValue('/test/project/.perplexity/tmp'),
      },
  ...
  async function* createStreamFromEvents(
    events: ServerPerplexityStreamEvent[],
  ...
  it('should process input and write text output', async () => {
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Hello' },
      { type: PerplexityEventType.Content, value: ' World' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockPerplexityClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockPerplexityClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerPerplexityStreamEvent = {
    ...
    const firstCallEvents: ServerPerplexityStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Final answer' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockPerplexityClient.sendMessageStream
    ...
    expect(mockPerplexityClient.sendMessageStream).toHaveBeenCalledTimes(2);
    ...
    expect(mockPerplexityClient.sendMessageStream).toHaveBeenNthCalledWith(
      1,
      [{ text: 'Use a tool' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerPerplexityStreamEvent[] = [
      {
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockPerplexityClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the Perplexity client
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Summary complete.' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Hello World' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerPerplexityStreamEvent[] = [
      toolCallEvent,
      {
        type: PerplexityEventType.Finished,
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Response from command' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Response to unknown' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Acknowledged' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should emit stream-json envelopes when output format is stream-json', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Hello stream' },
      {
        type: PerplexityEventType.Finished,
  ...
  it.skip('should emit a single user envelope when userEnvelope is provided', async () => {
    ...
    mockPerplexityClient.sendMessageStream.mockReturnValue(
      createStreamFromEvents([
        { type: PerplexityEventType.Content, value: 'Handled once' },
        {
          type: PerplexityEventType.Finished,
  ...
  it('should include usage metadata and API duration in stream-json result', async () => {
    ...
    mockPerplexityClient.sendMessageStream.mockReturnValue(
      createStreamFromEvents([
        { type: PerplexityEventType.Content, value: 'All done' },
      ]),
    );
  ...
  it('should not emit user message when userMessage option is provided (stream-json input binding)', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Response from envelope' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should emit tool results as user messages in stream-json format', async () => {
    ...
    const firstCallEvents: ServerPerplexityStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Final response' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle thinking blocks in stream-json format', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      ...
      { type: PerplexityEventType.Content, value: 'Response text' },
      {
        type: PerplexityEventType.Finished,
  ...
  it('should handle multiple tool calls in stream-json format', async () => {
    ...
    const toolCall1: ServerPerplexityStreamEvent = {
      type: PerplexityEventType.ToolCallRequest,
      value: {
        callId: 'tool-1',
        name: 'firstTool',
    ...
    const toolCall2: ServerPerplexityStreamEvent = {
      type: PerplexityEventType.ToolCallRequest,
      value: {
        callId: 'tool-2',
        name: 'secondTool',
    ...
    const secondCallEvents: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Combined response' },
      {
        type: PerplexityEventType.Finished,
  ...
  it('should handle userMessage with text content blocks in stream-json input mode', async () => {
    ...
    const events: ServerPerplexityStreamEvent[] = [
      { type: PerplexityEventType.Content, value: 'Response' },
      {
        type: PerplexityEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 3 } },
      },
    ...
    mockPerplexityClient.sendMessageStream.mockClear();
