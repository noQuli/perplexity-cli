...
import { Storage } from '@perplexity-cli/perplexity-cli-core';
  ...
  describe('Extension Command Loading', () => {
    it('loads commands from active extensions', async () => {
      ...
      const extensionDir = path.join(
        process.cwd(),
        '.perplexity/extensions/test-ext',
      );
      ...
      mock({
        ...
        [extensionDir]: {
          'perplexity-extension.json': JSON.stringify({
            name: 'test-ext',
            version: '1.0.0',
          }),
          commands: {
    ...
    it('only loads commands from active extensions', async () => {
      const extensionDir1 = path.join(
        process.cwd(),
        '.perplexity/extensions/active-ext',
      );
      const extensionDir2 = path.join(
        process.cwd(),
        '.perplexity/extensions/inactive-ext',
      );
    ...
    it('handles missing extension commands directory gracefully', async () => {
      const extensionDir = path.join(
        process.cwd(),
        '.perplexity/extensions/no-commands',
      );
    ...
    it('handles nested command structure in extensions', async () => {
      const extensionDir = path.join(process.cwd(), '.perplexity/extensions/a');
