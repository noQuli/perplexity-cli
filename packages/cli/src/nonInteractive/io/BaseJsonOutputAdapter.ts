...
export interface JsonOutputAdapterInterface extends MessageEmitter {
  ...
  processEvent(event: ServerPerplexityStreamEvent): void;
...
export abstract class BaseJsonOutputAdapter {
   ...
   *
   * @param metadata - Optional usage metadata from Perplexity API
  ...
  /**
   * Processes a stream event from the Perplexity API.
   * This is a shared implementation used by both streaming and non-streaming adapters.
   *
   * @param event - Stream event from Perplexity API
   */
  processEvent(event: ServerPerplexityStreamEvent): void {
    ...
    switch (event.type) {
      case PerplexityEventType.Content:
        this.appendText(state, event.value, null);
        break;
      case PerplexityEventType.Citation:
        if (typeof event.value === 'string') {
          this.appendText(state, `\n${event.value}`, null);
        }
        break;
      case PerplexityEventType.Thought:
        this.appendThinking(
          state,
          event.value.subject,
