...
import { isGitRepository } from '@perplexity-cli/perplexity-cli-core';

vi.mock('@perplexity-cli/perplexity-cli-core', () => ({
...
describe('getInstallationInfo', () => {
  ...
  it('should detect running via npx', () => {
    const npxPath = `/Users/test/.npm/_npx/12345/bin/perplexity`;
  ...
  it('should detect running via pnpx', () => {
    const pnpxPath = `/Users/test/.pnpm/_pnpx/12345/bin/perplexity`;
  ...
  it('should detect running via bunx', () => {
    const bunxPath = `/Users/test/.bun/install/cache/12345/bin/perplexity`;
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/usr/local/bin/perplexity';
    process.argv[1] = cliPath;
    mockedRealPathSync.mockReturnValue(cliPath);
    mockedExecSync.mockReturnValue(Buffer.from('perplexity-cli')); // Simulate successful command

    const info = getInstallationInfo(projectRoot, false);

    expect(mockedExecSync).toHaveBeenCalledWith(
      'brew list -1 | grep -q "^perplexity-code$"',
      { stdio: 'ignore' },
    );
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@perplexity-cli/perplexity-cli/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'pnpm add -g @perplexity-cli/perplexity-cli@latest',
    );
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@perplexity-cli/perplexity-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'yarn global add @perplexity-cli/perplexity-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/bin/perplexity`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'bun add -g @perplexity-cli/perplexity-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect local installation and identify yarn from lockfile', () => {
    const localPath = `${projectRoot}/node_modules/.bin/perplexity`;
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    const globalPath = `/usr/local/bin/perplexity`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'npm install -g @perplexity-cli/perplexity-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
