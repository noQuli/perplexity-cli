...
describe('computeWindowTitle', () => {
  ...
  it('should use default Perplexity title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Perplexity - my-project');
  });
  ...
  it('should handle folder names with control characters', () => {
    ...
    expect(result).toBe('Perplexity - projectname');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Perplexity - project-name_v1.0');
  });
});
