...
import { IdeClient, AuthType } from '@perplexity-cli/perplexity-cli-core';
 ...
 * For bug reports, removes 'perplexity-' or 'perplexity-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'perplexity-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove perplexity- prefix
  if (stripPrefix) {
    return sandbox.replace(/^perplexity-(?:code-)?/, '');
  }
