...
describe('handleAutoUpdate', () => {
  ...
  beforeEach(() => {
    ...
    mockUpdateInfo = {
      update: {
        ...
        current: '1.0.0',
        type: 'major',
        name: '@perplexity-cli/perplexity-cli',
      },
  ...
  it('should emit "update-received" but not update if auto-updates are disabled', () => {
    ...
    mockGetInstallationInfo.mockReturnValue({
      updateCommand: 'npm i -g @perplexity-cli/perplexity-cli@latest',
  ...
  it('should emit "update-failed" when the update process fails', async () => {
    ...
    expect(mockUpdateEventEmitter.emit).toHaveBeenCalledWith('update-failed', {
      message:
        'Automatic update failed. Please try updating manually. (command: npm i -g @perplexity-cli/perplexity-cli@2.0.0, stderr: An error occurred)',
    });
  });
  ...
  it('should use the "@nightly" tag for nightly updates', async () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      'npm i -g @perplexity-cli/perplexity-cli@nightly',
      {
        shell: true,
        stdio: 'pipe',
      },
