...
import {
  ...
  extractUsageFromPerplexityClient,
...
describe('extractUsageFromPerplexityClient', () => {
  it('should return undefined for null client', () => {
    expect(extractUsageFromPerplexityClient(null)).toBeUndefined();
  });

  it('should return undefined for non-object client', () => {
    expect(extractUsageFromPerplexityClient('not an object')).toBeUndefined();
  });

  it('should return undefined when getChat is not a function', () => {
    const client = { getChat: 'not a function' };
    expect(extractUsageFromPerplexityClient(client)).toBeUndefined();
  });
  ...
  it('should extract usage from latest response with usageMetadata', () => {
    ...
    const result = extractUsageFromPerplexityClient(client);
    expect(result).toEqual({
      input_tokens: 100,
      output_tokens: 200,
      total_tokens: 300,
...
describe('buildSystemMessage', () => {
  ...
  it('should use unknown version when getCliVersion returns null', async () => {
    ...
    expect(result.perplexity_code_version).toBe('unknown');
  });
});
