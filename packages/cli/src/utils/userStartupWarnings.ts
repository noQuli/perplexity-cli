...
import { canUseRipgrep } from '@perplexity-cli/perplexity-cli-core';
  ...
  check: async (options: WarningCheckOptions) => {
    try {
      ...
      if (workspaceRealPath === homeRealPath) {
        return 'You are running Perplexity Code in your home directory. It is recommended to run in a project-specific directory.';
      }
      return null;
    } catch (_err: unknown) {
...
const rootDirectoryCheck: WarningCheck = {
  ...
  check: async (options: WarningCheckOptions) => {
    try {
      ...
      const errorMessage =
        'Warning: You are running Perplexity Code in the root directory. Your entire folder structure will be used for context. It is strongly recommended to run in a project-specific directory.';
