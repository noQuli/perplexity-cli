...
/**
 * Extracts usage metadata from the Perplexity client's debug responses.
 *
 * @param perplexityClient - The Perplexity client instance
 * @returns Usage information or undefined if not available
 */
export function extractUsageFromPerplexityClient(
  perplexityClient: unknown,
): Usage | undefined {
  if (
    !perplexityClient ||
    typeof perplexityClient !== 'object' ||
    typeof (perplexityClient as { getChat?: unknown }).getChat !== 'function'
  ) {
    return undefined;
  }

  try {
    const chat = (perplexityClient as { getChat: () => unknown }).getChat();
    if (
      !chat ||
      typeof chat !== 'object' ||
...
export async function buildSystemMessage(
  ...
  const systemMessage: CLISystemMessage = {
    ...
    perplexity_code_version: config.getCliVersion() || 'unknown',
    agents: agentNames,
  };
