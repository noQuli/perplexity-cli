...
import { FatalSandboxError } from '@perplexity-cli/perplexity-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'perplexity-code-sandbox';
const SANDBOX_NETWORK_NAME = 'perplexity-code-sandbox';
const SANDBOX_PROXY_NAME = 'perplexity-code-sandbox-proxy';
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which perplexity)`
        : 'perplexity';
...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // start and set up proxy if PERPLEXITY_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['PERPLEXITY_SANDBOX_PROXY_COMMAND'];
    ...
    // determine full path for perplexity-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under perplexity-cli repo
    //
    // note this can only be done with binary linked from perplexity-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('perplexity-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed perplexity binary; ' +
            'run `npm link ./packages/cli` under perplexity-cli repo to switch to linked binary.',
        );
      } else {
        ...
        const buildScriptArgs: string[] = ['scripts/build_sandbox.js', '-s'];
        if (isCustomProjectSandbox) {
          console.error(`using ${projectSandboxDockerfile} for sandbox`);
          buildScriptArgs.push(
            '-f',
        ...
        execFileSync('node', buildScriptArgs, {
          cwd: gcRoot,
          stdio: 'inherit',
          env: {
            ...process.env,
            PERPLEXITY_SANDBOX: config.command,
          },
        });
      }
    }
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the perplexity-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify perplexity-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    // PERPLEXITY_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['PERPLEXITY_SANDBOX_PROXY_COMMAND'];
    ...
    const isIntegrationTest =
      process.env['PERPLEXITY_CLI_INTEGRATION_TEST'] === 'true';
    let containerName;
    if (isIntegrationTest) {
      containerName = `perplexity-cli-integration-test-${randomBytes(
        4,
      ).toString('hex')}`;
    ...
    // copy PERPLEXITY_CLI_TEST_VAR for integration tests
    if (process.env['PERPLEXITY_CLI_TEST_VAR']) {
      args.push(
        '--env',
        `PERPLEXITY_CLI_TEST_VAR=${process.env['PERPLEXITY_CLI_TEST_VAR']}`,
      );
    }

    // copy PERPLEXITY_API_KEY(s)
    ...
    // copy PERPLEXITY_MODEL
    if (process.env['PERPLEXITY_MODEL']) {
      args.push('--env', `PERPLEXITY_MODEL=${process.env['PERPLEXITY_MODEL']}`);
    }
    ...
    if (process.env['PERPLEXITY_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
      ...
      // finally switch to that user to run the perplexity process. This is
      // necessary on Linux to ensure the user exists within the
      // container's /etc/passwd file, which is required by os.userInfo().
      const username = 'perplexity';
