...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-perplexity-cli/releases/latest`;
    ...
    console.debug(
      `Failed to determine latest run-perplexity-cli release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest run-perplexity-cli release on GitHub.`,
    );
  }
};
