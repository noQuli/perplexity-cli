...
export const GITHUB_WORKFLOW_PATHS = [
  'perplexity-dispatch/perplexity-dispatch.yml',
  'perplexity-assistant/perplexity-invoke.yml',
  'issue-triage/perplexity-triage.yml',
  'issue-triage/perplexity-scheduled-triage.yml',
  'pr-review/perplexity-review.yml',
];
...
// Add Perplexity CLI specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.perplexity/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/google-github-actions/run-perplexity-cli/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/google-github-actions/run-perplexity-cli/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with Perplexity.',
        command,
      },
    };
  },
