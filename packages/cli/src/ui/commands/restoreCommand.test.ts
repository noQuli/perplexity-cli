...
import type { Config, GitService } from '@perplexity-cli/perplexity-cli-core';
...
describe('restoreCommand', () => {
  ...
  beforeEach(async () => {
    ...
    perplexityTempDir = path.join(testRootDir, '.perplexity');
    checkpointsDir = path.join(perplexityTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempDir: vi.fn().mockReturnValue(perplexityTempDir),
      },
      getPerplexityClient: vi.fn().mockReturnValue({
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .perplexity directory path.',
      });
    });
