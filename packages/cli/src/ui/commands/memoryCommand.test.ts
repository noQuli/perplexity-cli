...
describe('memoryCommand', () => {
  ...
  describe('/memory show', () => {
    ...
    let mockGetPerplexityMdFileCount: Mock;

    beforeEach(() => {
      showCommand = getSubCommand('show');

      mockGetUserMemory = vi.fn();
      mockGetPerplexityMdFileCount = vi.fn();

      mockContext = createMockCommandContext({
        services: {
          config: {
            getUserMemory: mockGetUserMemory,
            getPerplexityMdFileCount: mockGetPerplexityMdFileCount,
          },
        },
      });
    });
    ...
    it('should display a message if memory is empty', async () => {
      ...
      mockGetPerplexityMdFileCount.mockReturnValue(0);
    ...
    it('should display the memory content and file count if it exists', async () => {
      ...
      mockGetPerplexityMdFileCount.mockReturnValue(1);
  ...
  describe('/memory refresh', () => {
    ...
    let mockSetPerplexityMdFileCount: Mock;

    beforeEach(() => {
      refreshCommand = getSubCommand('refresh');
      mockSetUserMemory = vi.fn();
      mockSetPerplexityMdFileCount = vi.fn();
      const mockConfig = {
        setUserMemory: mockSetUserMemory,
        setPerplexityMdFileCount: mockSetPerplexityMdFileCount,
    ...
    it('should display success message when memory is refreshed with content', async () => {
      ...
      expect(mockSetPerplexityMdFileCount).toHaveBeenCalledWith(
    ...
    it('should display success message when memory is refreshed with no content', async () => {
      ...
      expect(loadServerHierarchicalMemory).toHaveBeenCalledOnce();
      expect(mockSetUserMemory).toHaveBeenCalledWith('');
      expect(mockSetPerplexityMdFileCount).toHaveBeenCalledWith(0);
    ...
    it('should display an error message if refreshing fails', async () => {
      ...
      expect(mockSetPerplexityMdFileCount).not.toHaveBeenCalled();
