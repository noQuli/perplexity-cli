...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      `set -eEuo pipefail`,
      `fakeOpenCommand "https://github.com/google-github-actions/run-perplexity-cli`,
    ];
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.perplexity/');
...
describe('updateGitignore', () => {
  ...
  it('creates a new .gitignore file when none exists', async () => {
    ...
    expect(content).toBe('.perplexity/\ngha-creds-*.json\n');
  });
  ...
  it('does not add duplicate entries', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = '.perplexity/\nsome-other-file\ngha-creds-*.json\n';
    await fs.writeFile(gitignorePath, existingContent);
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .perplexity/ entry
    expect((content.match(/\.perplexity\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .perplexity/ folder',
      'my-app.perplexity/config',
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.perplexity/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.perplexity/config');
