...
export const toolsCommand: SlashCommand = {
  name: 'tools',
  description: 'List available Perplexity CLI tools. Usage: /tools [desc]',
  ...
  action: async (context: CommandContext, args?: string): Promise<void> => {
    ...
    const perplexityTools = tools.filter((tool) => !('serverName' in tool));

    const toolsListItem: HistoryItemToolsList = {
      type: MessageType.TOOLS_LIST,
      tools: perplexityTools.map((tool) => ({
        name: tool.name,
