...
describe('initCommand', () => {
  ...
  const DEFAULT_CONTEXT_FILENAME = 'PERPLEXITY.md';
  const perplexityMdPath = path.join(targetDir, DEFAULT_CONTEXT_FILENAME);
  ...
  it(`should create ${DEFAULT_CONTEXT_FILENAME} and submit a prompt if it does not exist`, async () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(perplexityMdPath, '', 'utf8');
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        content: expect.stringContaining(
          'You are Perplexity Code, an interactive CLI agent',
        ),
      }),
    );
  });
