...
import { getProjectSummaryPrompt } from '@perplexity-cli/perplexity-cli-core';
...
export const summaryCommand: SlashCommand = {
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    try {
      ...
      const chat = perplexityClient.getChat();
      ...
      const response = await perplexityClient.generateContent(
        [
          ...conversationContext,
          {
            role: 'user',
      ...
      // Ensure .perplexity directory exists
      const projectRoot = config.getProjectRoot();
      const perplexityDir = path.join(projectRoot, '.perplexity');
      try {
        await fsPromises.mkdir(perplexityDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(perplexityDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      const completedSummaryItem: HistoryItemSummary = {
        ...
        summary: {
          isPending: false,
          stage: 'completed',
          filePath: '.perplexity/PROJECT_SUMMARY.md',
        },
      };
