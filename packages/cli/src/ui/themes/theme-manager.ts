...
import { PerplexityLight } from './perplexity-light.js';
import { PerplexityDark } from './perplexity-dark.js';
...
export const DEFAULT_THEME: Theme = PerplexityDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate Perplexity themes
    const perplexityThemes = builtInThemes.filter(
      (theme) =>
        theme.name === PerplexityLight.name ||
        theme.name === PerplexityDark.name,
    );
    ...
    // Combine Perplexity themes first, then sorted others
    return [...perplexityThemes, ...sortedOtherThemes];
  }
