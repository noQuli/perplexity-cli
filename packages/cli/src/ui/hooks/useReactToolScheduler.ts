...
import { CoreToolScheduler } from '@perplexity-cli/perplexity-cli-core';
...
export type TrackedScheduledToolCall = ScheduledToolCall & {
  responseSubmittedToPerplexity?: boolean;
};
    ...
    (updatedCoreToolCalls: ToolCall[]) => {
      setToolCallsForDisplay((prevTrackedCalls) =>
        updatedCoreToolCalls.map((coreTc) => {
          ...
          const responseSubmittedToPerplexity =
            existingTrackedCall?.responseSubmittedToPerplexity ?? false;
      ...
      setToolCallsForDisplay((prevCalls) =>
        prevCalls.map((tc) =>
          callIdsToMark.includes(tc.request.callId)
            ? { ...tc, responseSubmittedToPerplexity: true }
            : tc,
        ),
      );
    },
