...
// Skip all vision auto-switch tests - Perplexity doesn't have vision models
describe.skip('useVisionAutoSwitch helpers', () => {
  describe('shouldOfferVisionSwitch', () => {
    it('returns false when authType is not PERPLEXITY_API_KEY', () => {
    ...
    it('returns true when image parts exist, PERPLEXITY_API_KEY, and model is not vision', () => {
      const parts: PartListUnion = [
        { text: 'hello' },
    ...
    it('returns false when authType is not PERPLEXITY_API_KEY in YOLO mode context', () => {
      const parts: PartListUnion = [
        { inlineData: { mimeType: 'image/png', data: '...' } },
      ];
      const result = shouldOfferVisionSwitch(
...
// Skip all vision auto-switch hook tests - Perplexity doesn't have vision models
describe.skip('useVisionAutoSwitch hook', () => {
  type AddItemFn = (
  ...
  it('returns shouldProceed=true immediately for continuations', async () => {
    const config = createMockConfig(AuthType.PERPLEXITY_API_KEY, 'sonar-pro');
  ...
  it('does nothing when authType is not PERPLEXITY_API_KEY', async () => {
    const config = createMockConfig(AuthType.USE_VERTEX_AI, 'sonar-pro');
  ...
  it('applies a one-time override and returns originalModel, then restores', async () => {
    const initialModel = 'sonar-pro';
    const config = createMockConfig(AuthType.PERPLEXITY_API_KEY, initialModel);
  ...
  describe('YOLO mode behavior', () => {
    ...
    it('does not switch in YOLO mode when authType is not PERPLEXITY_API_KEY', async () => {
      const config = createMockConfig(
        AuthType.USE_VERTEX_AI,
        'sonar-pro',
        ApprovalMode.YOLO,
  ...
  describe('VLM switch mode default behavior', () => {
    it('should automatically switch once when vlmSwitchMode is "once"', async () => {
      ...
      expect(switchResult.originalModel).toBe('sonar-pro');
