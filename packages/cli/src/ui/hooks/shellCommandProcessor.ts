...
function addShellCommandToPerplexityHistory(
        ...
        try {
              ...
              switch (event.type) {
                case 'data':
                  ...
                  if (config.getDebugMode()) {
                    const chunkType = Array.isArray(event.chunk)
                      ? 'AnsiOutput'
                      : typeof event.chunk;
                    const chunkSize = Array.isArray(event.chunk)
                      ? event.chunk.length
                      : (event.chunk as string).length;
                    onDebugMessage(
                      `[ShellProcessor] Rx chunk: type=${chunkType}, size=${chunkSize}, isPty=${config.getShouldUseNodePtyShell()}`,
                    );
                  }
          ...
          result
            .then((result: ShellExecutionResult) => {
              ...
              addShellCommandToPerplexityHistory(
                perplexityClient,
                rawQuery,
                finalOutput,
              );
