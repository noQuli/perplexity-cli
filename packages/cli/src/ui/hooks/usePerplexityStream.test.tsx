...
import { usePerplexityStream } from './usePerplexityStream.js';
...
const MockedPerplexityClientClass = vi.hoisted(() =>
...
vi.mock('@perplexity-cli/perplexity-cli-core', async (importOriginal) => {
  ...
  return {
    ...actualCoreModule,
    GitService: vi.fn(),
    PerplexityClient: MockedPerplexityClientClass,
...
// --- Tests for usePerplexityStream Hook ---
describe('usePerplexityStream', () => {
  ...
  beforeEach(() => {
    ...
    mockAddItem = vi.fn();
    // Define the mock for getPerplexityClient
    const mockGetPerplexityClient = vi.fn().mockImplementation(() => {
      // MockedPerplexityClientClass is defined in the module scope by the previous change.
      // It will use the mockStartChat and mockSendMessageStream that are managed within beforeEach.
      const clientInstance = new MockedPerplexityClientClass(mockConfig);
    ...
    mockConfig = {
      ...
      getPerplexityClient: mockGetPerplexityClient,
    ...
    // Reset mocks for PerplexityClient instance methods (startChat and sendMessageStream)
    // The PerplexityClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // PerplexityChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.perplexity/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = (
    ...
    const client = perplexityClient || mockConfig.getPerplexityClient();
    ...
    const toolCalls: TrackedToolCall[] = [
      {
        ...
        status: 'success',
        responseSubmittedToPerplexity: false,
        response: {
          callId: 'call1',
  ...
  it('should submit tool responses when all tool calls are completed and ready', async () => {
    ...
    renderHook(() =>
      usePerplexityStream(
        new MockedPerplexityClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedPerplexityClientClass(mockConfig);
  ...
  describe('User Cancellation', () => {
    ...
    it('should call onCancelSubmit handler when escape is pressed', async () => {
      ...
      const { result } = renderHook(() =>
        usePerplexityStream(
          mockConfig.getPerplexityClient(),
          [],
          mockAddItem,
          mockConfig,
          mockLoadedSettings,
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Perplexity when a command is handled without a tool call', async () => {
    ...
    it('should call Perplexity with prompt content when slash command returns a `submit_prompt` action', async () => {
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = renderHook(() =>
        usePerplexityStream(
          new MockedPerplexityClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPerplexityEventType.Content,
            value: 'This is a truncated response...',
          };
          yield {
            type: ServerPerplexityEventType.Finished,
  ...
  it('should process @include commands, adding user turn after processing to prevent race conditions', async () => {
    ...
    const { result } = renderHook(() =>
      usePerplexityStream(
        mockConfig.getPerplexityClient() as PerplexityClient,
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  describe('Thought Reset', () => {
    it('should reset thought to null when starting a new prompt', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPerplexityEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerPerplexityEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerPerplexityEventType.Error,
  ...
  describe('Concurrent Execution Prevention', () => {
    it('should prevent concurrent submitQuery calls', async () => {
      ...
      const firstStream = (async function* () {
        ...
        yield { type: ServerPerplexityEventType.Finished, value: 'STOP' };
      })();
  ...
  describe('Vision Auto Switch Integration', () => {
    it('should call handleVisionSwitch and proceed to send when allowed', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerPerplexityEventType.Content, value: 'ok' };
          yield { type: ServerPerplexityEventType.Finished, value: 'STOP' };
        })(),
      );
  ...
  describe('Model restore on completion and errors', () => {
    it('should restore model after successful stream completion', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerPerplexityEventType.Content, value: 'content' };
          yield { type: ServerPerplexityEventType.Finished, value: 'STOP' };
        })(),
      );
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getPerplexityClient = vi.fn().mockReturnValue({
        ...new MockedPerplexityClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerPerplexityEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getPerplexityClient = vi.fn().mockReturnValue(mockClient);
