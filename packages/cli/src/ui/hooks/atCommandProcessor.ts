...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const perplexityIgnored =
      respectFileIgnore.respectPerplexityIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectPerplexityIgnore: true,
      });

    if (gitIgnored || perplexityIgnored) {
      const reason =
        gitIgnored && perplexityIgnored
          ? 'both'
          : gitIgnored
            ? 'git'
            : 'perplexity';
      ...
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and perplexity'
          : reason === 'git'
            ? 'git-ignored'
            : 'perplexity-ignored';
  ...
  const totalIgnored =
    ...
    ignoredByReason['perplexity'].length +
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['perplexity'].length) {
      messages.push(
        `Perplexity-ignored: ${ignoredByReason['perplexity'].join(', ')}`,
      );
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_perplexity_ignore: respectFileIgnore.respectPerplexityIgnore,
    },
