...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectPerplexityIgnore: () => true,
  ...
  describe('perplexity-ignore filtering', () => {
    it('should skip perplexity-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.perplexityignore'),
        'build/output.js',
      );
      const perplexityIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${perplexityIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${perplexityIgnoredFile} is perplexity-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nPerplexity-ignored: ${perplexityIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .perplexityignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.perplexityignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed perplexity-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.perplexityignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${perplexityIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${validFile} @${perplexityIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${validFile}:\n` },
