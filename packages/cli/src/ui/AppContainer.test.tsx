...
vi.mock('./hooks/usePerplexityStream.js');
...
describe('AppContainer State Management', () => {
  ...
  const mockedUsePerplexityStream = usePerplexityStream as Mock;
  ...
  beforeEach(() => {
    ...
    vi.mock('../utils/windowTitle.js', async () => ({
      computeWindowTitle: vi.fn(
        (folderName: string) =>
          // Default behavior: return "Perplexity - {folderName}" unless CLI_TITLE is set
          process.env['CLI_TITLE'] || `Perplexity - ${folderName}`,
      ),
    }));
    ...
    mockedUsePerplexityStream.mockReturnValue({
      streamingState: 'idle',
      submitQuery: vi.fn(),
      initError: null,
      pendingHistoryItems: [],
    ...
    // Mock PerplexityClient to prevent unhandled errors from TaskTool.refreshSubagents
    const mockPerplexityClient: Partial<PerplexityClient> = {
      initialize: vi.fn().mockResolvedValue(undefined),
      setTools: vi.fn().mockResolvedValue(undefined),
      isInitialized: vi.fn().mockReturnValue(false), // Return false to prevent setTools from being called
    };
    vi.spyOn(mockConfig, 'getPerplexityClient').mockReturnValue(
      mockPerplexityClient as PerplexityClient,
    );
  ...
  describe('Quota and Fallback Integration', () => {
    ...
    it('passes a valid proQuotaRequest to UIStateContext when provided by the hook', () => {
      ...
      const mockRequest = {
        failedModel: 'perplexity-pro',
        fallbackModel: 'perplexity-flash',
        resolve: vi.fn(),
      };
      mockedUseQuotaAndFallback.mockReturnValue({
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should update terminal title with default text when in Idle state and no thought subject', () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]2;${'Perplexity - workspace'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    it('should use CLI_TITLE environment variable when set', () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom Perplexity Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]2;${'Custom Perplexity Title'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
