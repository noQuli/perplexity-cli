...
import { loadHierarchicalPerplexityMemory } from '../config/config.js';
...
import { usePerplexityStream } from './hooks/usePerplexityStream.js';
...
export const AppContainer = (props: AppContainerProps) => {
  ...
  const [perplexityMdFileCount, setPerplexityMdFileCount] = useState<number>(
    initializationResult.perplexityMdFileCount,
  );
  ...
  const getEffectiveModel = useCallback(() => {
    if (config.isInFallbackMode()) {
      return DEFAULT_PERPLEXITY_FLASH_MODEL;
    }
    ...
    historyManager.addItem(
      {
        ...
        text: 'Refreshing hierarchical memory (PERPLEXITY.md or other context files)...',
      },
      Date.now(),
    );
    try {
      const { memoryContent, fileCount } =
        await loadHierarchicalPerplexityMemory(
          process.cwd(),
      ...
      config.setPerplexityMdFileCount(fileCount);
      setPerplexityMdFileCount(fileCount);
  ...
  const {
    streamingState,
    submitQuery,
    pendingHistoryItems: pendingPerplexityHistoryItems,
    thought,
  ...
  useEffect(() => {
    if (
      ...
      perplexityClient?.isInitialized?.()
    ) {
  ...
  // Update terminal title with Perplexity CLI status and thoughts
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because Perplexity CLI is already doing that elsewhere
  }, [
    streamingState,
    thought,
    settings.merged.ui?.showStatusInTitle,
  ...
  const pendingHistoryItems = useMemo(
    ...
    [pendingSlashCommandHistoryItems, pendingPerplexityHistoryItems],
  );
  ...
  const uiState: UIState = useMemo(
    () => ({
      ...
      confirmUpdateExtensionRequests,
      loopDetectionConfirmationRequest,
      perplexityMdFileCount,
      streamingState,
      initError: null, // No initialization error tracking currently implemented
      pendingPerplexityHistoryItems,
      thought,
      shellModeActive,
      userMessages,
