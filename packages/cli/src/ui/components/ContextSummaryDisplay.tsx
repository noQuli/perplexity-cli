...
export const ContextSummaryDisplay: React.FC<ContextSummaryDisplayProps> = ({
  ...
  const perplexityMdText = (() => {
    if (perplexityMdFileCount === 0) {
      return '';
    }
    const allNamesTheSame = new Set(contextFileNames).size < 2;
    const name = allNamesTheSame ? contextFileNames[0] : 'context';
    return perplexityMdFileCount === 1
      ? t('{{count}} {{name}} file', {
          count: String(perplexityMdFileCount),
          name,
        })
      : t('{{count}} {{name}} files', {
          count: String(perplexityMdFileCount),
  ...
  const summaryParts = [openFilesText, perplexityMdText, mcpText].filter(
    Boolean,
  );
