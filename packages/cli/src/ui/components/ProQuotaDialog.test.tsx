...
describe('ProQuotaDialog', () => {
  ...
  it('should render with correct title and options', () => {
    const { lastFrame } = render(
      <ProQuotaDialog
        failedModel="perplexity-2.5-pro"
        fallbackModel="perplexity-2.5-flash"
        onChoice={() => {}}
      />,
    );

    const output = lastFrame();
    expect(output).toContain('Pro quota limit reached for perplexity-2.5-pro.');
    ...
    expect(RadioButtonSelect).toHaveBeenCalledWith(
      expect.objectContaining({
        items: [
          ...
          {
            label: `Continue with perplexity-2.5-flash`,
            value: 'continue',
            key: 'continue',
          },
        ],
