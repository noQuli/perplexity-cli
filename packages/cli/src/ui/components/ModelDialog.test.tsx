...
const renderComponent = (
  ...
  const mockConfig = contextValue
    ? ({
        ...
        setModel: vi.fn(),
        getAuthType: vi.fn(() => 'perplexity-api-key'),
...
describe('<ModelDialog />', () => {
  ...
  it('passes all model options to DescriptiveRadioButtonSelect', () => {
    ...
    expect(props.items).toHaveLength(AVAILABLE_MODELS_PERPLEXITY.length);
    // First model should be sonar-pro (MAINLINE_CODER)
    expect(props.items[0].value).toBe(SONAR_PRO);
    // Second model should be sonar
    expect(props.items[1].value).toBe(SONAR);
  ...
  it('initializes with the model from ConfigContext', () => {
    ...
    // MAINLINE_VLM is SONAR_PRO which is at index 0
  ...
  it('updates initialIndex when config context changes', () => {
    ...
    const mockGetAuthType = vi.fn(() => 'perplexity-api-key');
    ...
    // Change to SONAR (second model in list)
    mockGetModel.mockReturnValue(SONAR);
    const newMockConfig = {
      getModel: mockGetModel,
      getAuthType: mockGetAuthType,
    } as unknown as Config;
