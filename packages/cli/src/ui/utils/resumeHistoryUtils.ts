/**
 * @license
 * Copyright 2025 Perplexity Code
  ...
  for (const record of conversation.messages) {
    ...
    switch (record.type) {
      ...
      case 'assistant': {
        ...
        // If there's thought content, add it as a perplexity_thought message
        if (thoughtText) {
          ...
          items.push({ type: 'perplexity_thought', text: thoughtText });
        }

        // If there's text content, add it as a perplexity message
        if (text) {
          ...
          items.push({ type: 'perplexity', text });
        }
