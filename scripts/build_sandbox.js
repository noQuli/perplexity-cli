...
if (!image.length) {
  console.warn(
    'No default image tag specified in perplexity-cli/packages/cli/package.json',
  );
}
...
console.log('packing @perplexity-cli/perplexity-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'perplexity-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @perplexity-cli/perplexity-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @perplexity-cli/perplexity-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'perplexity-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @perplexity-cli/perplexity-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(
    cliPackageDir,
    'dist',
    `perplexity-cli-perplexity-cli-${packageVersion}.tgz`,
  ),
  0o755,
);
chmodSync(
  join(
    corePackageDir,
    'dist',
    `perplexity-cli-perplexity-cli-core-${packageVersion}.tgz`,
  ),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `perplexity-auth-${Date.now()}.json`);
  ...
  const imageTag =
    process.env.PERPLEXITY_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
