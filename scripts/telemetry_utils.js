...
// User-level .perplexity directory in home
const USER_PERPLEXITY_DIR = path.join(os.homedir(), '.perplexity');
// Project-level .perplexity directory in the workspace
const WORKSPACE_PERPLEXITY_DIR = path.join(projectRoot, '.perplexity');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.perplexity/tmp
...
// Workspace settings remain in the project's .perplexity directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_PERPLEXITY_DIR,
  'settings.json',
);
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `perplexity-cli-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      ...
      [
        '-sL',
        '-H',
        'User-Agent: perplexity-cli-dev-script',
        '-o',
        tmpFile,
        url,
      ],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'perplexity-cli-telemetry-'),
  );
