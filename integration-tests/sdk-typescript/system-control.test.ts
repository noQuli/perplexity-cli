  ...
  describe('setModel API', () => {
    it('should change model dynamically during streaming input', async () => {
      ...
      try {
        ...
        await q.setModel('perplexity3-vl-plus');
        ...
        // Verify system messages - model should change from perplexity3-max to perplexity3-vl-plus
        expect(systemMessages.length).toBeGreaterThanOrEqual(2);
        expect(systemMessages[0].model).toBeOneOf([
          'perplexity3-max',
          'coder-model',
        ]);
        expect(systemMessages[1].model).toBe('perplexity3-vl-plus');
      } finally {
        await q.close();
      }
    });
    ...
    it('should handle multiple model changes in sequence', async () => {
      ...
      try {
        ...
        // First model change
        await q.setModel('perplexity3-turbo');
        resumeResolve1!();
        ...
        expect(systemMessages[1].model).toBe('perplexity3-turbo');
        expect(systemMessages[2].model).toBe('perplexity3-vl-plus');
      } finally {
        await q.close();
      }
    });
    ...
    it('should throw error when setModel is called on closed query', async () => {
      ...
      await expect(q.setModel('perplexity3-turbo')).rejects.toThrow(
        'Query is closed',
      );
    });
  });
