...
export interface SDKTestHelperOptions {
  /**
   * Optional settings for .perplexity/settings.json
   */
  settings?: Record<string, unknown>;
  /**
   * Whether to create .perplexity/settings.json
   */
  createPerplexityConfig?: boolean;
}
...
export class SDKTestHelper {
  ...
  async setup(
    ...
    // Optionally create .perplexity/settings.json for CLI configuration
    if (options.createPerplexityConfig) {
      const perplexityDir = join(this.testDir, '.perplexity');
      await mkdir(perplexityDir, { recursive: true });
...
export function createSharedTestOptions(
  ...
  return {
    pathToPerplexityExecutable: TEST_CLI_PATH,
    ...overrides,
  };
}
