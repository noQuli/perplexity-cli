...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['PERPLEXITY_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    const perplexityDir = join(this.testDir, '.perplexity');
    mkdirSync(perplexityDir, { recursive: true });
    ...
    const settings = {
      ...
      sandbox:
        env.PERPLEXITY_SANDBOX !== 'false' ? env.PERPLEXITY_SANDBOX : false,
      ...options.settings, // Allow tests to override/add settings
    };
    writeFileSync(
      join(perplexityDir, 'settings.json'),
  ...
  /**
   * The command and args to use to invoke Perplexity CLI CLI. Allows us to switch
   * between using the bundled perplexity.js (the default) and using the installed
   * 'perplexity' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const isNpmReleaseTest =
      process.env.INTEGRATION_TEST_USE_INSTALLED_PERPLEXITY === 'true';
    const command = isNpmReleaseTest ? 'perplexity' : 'node';
    ...
    const promise = new Promise<string>((resolve, reject) => {
      child.on('close', (code: number) => {
        if (code === 0) {
          ...
          let result = stdout;
          if (env['PERPLEXITY_SANDBOX'] === 'podman') {
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `perplexity-cli.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'perplexity-cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'perplexity-cli.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'perplexity-cli.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `perplexity-cli.${metricName}`) {
              return metric;
            }
          }
        }
