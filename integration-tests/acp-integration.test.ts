...
const LIST_SIZE = 5;
const IS_SANDBOX =
  process.env['PERPLEXITY_SANDBOX'] &&
  process.env['PERPLEXITY_SANDBOX']!.toLowerCase() !== 'false';
...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
  ...
  it('receives available_commands_update with slash commands after session creation', async () => {
    ...
    try {
      ...
      // multi-step process (listing files, reading up to 10 files, generating PERPLEXITY.md)
