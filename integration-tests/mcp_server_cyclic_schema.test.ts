 ...
 * (in JSON, using the $ref keyword) and both the GenAI SDK and the Perplexity
 ...
 * the Perplexity API call would fail for such tools because the schema was
 * passed not as a JSON string but using the Perplexity API's tool parameter
 * schema object which has stricter typing and recursion restrictions.
 * If this test fails, it's likely because either the GenAI SDK or Perplexity API
 * has become more restrictive about the type of tool parameter schemas that
 * are accepted. If this occurs: Perplexity CLI previously attempted to detect
 * such tools and proactively remove them from the set of tools provided in
 * the Perplexity API call (as FunctionDeclaration objects). It may be appropriate
 * to resurrect that behavior but note that it's difficult to keep the
 * GCLI filters in sync with the Perplexity API restrictions and behavior.
 */
...
describe('mcp server with cyclic tool schema is detected', () => {
  ...
  it('perplexity api call should be successful with cyclic mcp tool schema', async () => {
